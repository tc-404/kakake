...
import {
  ...
  Clock, Timer, type LucideIcon,
} from 'lucide-react';
import { api } from '@/lib/api';
import { ColumnHistogramCard, SparklineCard } from '@/components/system-metric-cards';
    ...
    return (
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {['系统', 'CPU', '系统运行时长', 'kakake 运行时长'].map((l) => (
...
export function SystemMonitorPanel({ className }: { className?: string }) {
  ...
  // kakake 进程内存占用历史（与总内存叠加显示）
  const [procMemHistory, setProcMemHistory] = useState<number[]>([]);
  const procMemRef = useRef<number[]>([]);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        const nextProcMem = [...procMemRef.current, clampPercent(m.memory.nodePercent ?? m.memory.processPercent)].slice(-HISTORY_LEN);
        procMemRef.current = nextProcMem;
        setProcMemHistory(nextProcMem);
      } catch (e) {
    ...
    <div className={cn('space-y-6', className)}>
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          />
          {/* 内存趋势面积图：总内存（粉）叠加 kakake 进程占用（紫） */}
          <SparklineCard
            label="内存占用"
            history={memHistory}
            currentPercent={metrics?.memory.systemPercent ?? 0}
            history2={procMemHistory}
            current2={metrics?.memory.nodePercent ?? metrics?.memory.processPercent ?? 0}
            color2="#8B5CF6"
            legend={{ primary: '总内存', secondary: '进程' }}
            hint={
              metrics ? (
                <>
                  {/* 与曲线主色（粉）对齐；用固定色写死，不吃全局自定义字体色 */}
                  <span style={{ color: '#DB2777' }}>
                    总 {formatBytes(metrics.memory.systemUsedBytes)}/{formatBytes(metrics.memory.systemTotalBytes)}
                  </span>
                  <span className="mx-1">·</span>
                  {/* 与曲线副色（紫）对齐 */}
                  <span style={{ color: '#7C3AED' }}>
                    进程 {formatBytes(metrics.memory.nodeRssBytes ?? metrics.memory.processRssBytes)}（堆 {formatBytes(metrics.memory.nodeHeapUsedBytes ?? metrics.memory.processHeapUsedBytes)}）
                  </span>
                </>
              ) : (
                '采集中…'
      ...
      <DiskCard disk={metrics?.disk ?? null} disks={metrics?.disks} />
