...
export function ConnectionManageDialog({
  ...
  const showCopyAddress = conn && ((!isQq || isQqHttps) && !isWx);
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
              ...
              {isQqHttps && conn && (
                <>
                  ...
                  <div className="space-y-2 rounded-md border bg-muted/30 p-3">
                    ...
                    <code className="block break-all font-mono text-[11px]">{conn.listenUrl}</code>
        ...
        <DialogFooter className="!flex-row !justify-center gap-3 space-x-0 sm:!flex-row sm:!justify-center sm:space-x-0">
          <Button variant="outline" className="min-w-0 flex-1" onClick={onClose}>
            取消
          </Button>
