...
function AlertDialogOverlay({ className, ...props }: DivProps) {
  return (
    <Overlay
      className={cn(
        'kk-pop fixed inset-0 z-50 bg-black/10 backdrop-blur-md',
        className,
      )}
      {...props}
    />
...
function AlertDialogContent({ className, ...props }: DivProps) {
  return (
    <AlertDialogPortal>
      ...
      <Content
        className={cn(
          'fixed left-1/2 top-1/2 z-50 grid max-h-[min(85dvh,100dvh-2rem)] w-[calc(100vw-1.5rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto rounded-2xl kk-glass-2 p-5 shadow-[0_8px_32px_rgba(0,0,0,0.08)]',
          'kk-pop',
          'sm:p-6',
          className,
        )}
