...
import { ChevronDown, Download, Loader2, RefreshCw, Zap } from 'lucide-react';
...
function UpdatePopover({
  ...
  return createPortal(
    <>
      ...
      >
        {/* 头部：版本对比 + 状态 */}
        <div className="mb-3 flex items-center gap-2 text-sm tabular-nums">
          <span
            className={cn(
              'font-semibold',
        ...
        <div className="mt-3 flex flex-col gap-2">
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={!!pingMode}
              className={cn(
                'inline-flex h-8 flex-1 items-center justify-center gap-1.5 rounded-lg border border-white/40 bg-white/25 px-3 text-xs font-medium text-teal-700 transition-colors',
          ...
          <a
            ...
            className={cn(
              'inline-flex h-9 w-full items-center justify-center gap-1.5 rounded-lg bg-teal-500/15 px-3 text-sm font-semibold text-teal-700 transition-colors',
              'hover:bg-teal-500/25 active:scale-[0.99]',
              !selVersion && 'pointer-events-none opacity-50',
            )}
          >
            <Download className="h-4 w-4" />
            {selVersion ? `去下载该版本 · v${selVersion.version}` : '去下载该版本'}
          </a>
        </div>
      </div>
    </>,
