...
export function resolveConnVisual(conn: ConnectionStatus): {
  ...
  if (conn.type === 'qq_official') {
    const isHttps = conn.mode === 'https';
    if (conn.connected) {
      // Webhook 模式下“监听已就绪”不等于平台能推到：没被平台验证过就得说清楚
      if (isHttps && conn.webhookVerified === false) {
        return { key: 'waiting', label: '等待腾讯回调', variant: 'warning', failed: false };
      }
      return {
        key: 'connected',
        label: isHttps ? '回调已就绪' : '已连接腾讯',
        variant: 'success',
        failed: false,
      };
    }
    if (conn.reconnectAbandoned) {
    ...
    return {
      key: 'connecting',
      label: (conn.reconnectAttempts ?? 0) > 0 ? '重连中' : '连接中',
      variant: 'warning',
      failed: false,
    };
  }
  if (conn.type === 'kook') {
    if (!conn.kookReady) {
      return { key: 'waiting', label: '未配置 Token', variant: 'warning', failed: false };
    }
    if (conn.connected) {
      return { key: 'connected', label: '已连接 KOOK', variant: 'success', failed: false };
    }
  }
  if (conn.connected) {
    const label =
