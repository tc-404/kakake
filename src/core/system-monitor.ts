...
export type SystemMetrics = {
  ...
  /** 机器上枚举到的所有卷（Windows 为各盘符；其它平台通常仅项目所在卷） */
  disks: {
    usedPercent: number;
    usedBytes: number;
    totalBytes: number;
...
async function sampleDiskAt(rootPath: string): Promise<SystemMetrics['disk'] | null> {
  try {
    ...
    return { usedPercent, usedBytes, totalBytes, freeBytes, path: rootPath };
  } catch {
    return null;
  }
}
...
async function sampleDisk(rootPath: string): Promise<SystemMetrics['disk']> {
  const d = await sampleDiskAt(rootPath);
  if (d) return d;
  return { usedPercent: 0, usedBytes: 0, totalBytes: 0, freeBytes: 0, path: rootPath };
}

/**
 * Linux：从 /proc/mounts 枚举真实块设备卷（系统盘 + 数据盘），
 * 排除 tmpfs/proc/sysfs/overlay/cgroup 等伪文件系统与 snap 只读镜像。
 * 同一设备的多个挂载点（bind / 子卷）只保留挂载点最短的那个，避免重复计量。
 */
async function sampleLinuxDisks(): Promise<SystemMetrics['disk'][]> {
  let raw: string;
  try {
  ...
  // snap 包挂载的是只读 squashfs 镜像，不是数据盘
  const SKIP_FS = new Set(['squashfs']);
  const byDevice = new Map<string, string>(); // device -> 最短挂载点
  for (const line of raw.split('\n')) {
    if (!line) continue;
    const parts = line.split(/\s+/);
    if (parts.length < 3) continue;
    const device = parts[0];
    // /proc/mounts 把空格/制表符转义成 \040 \011，还原回来
    const mountpoint = parts[1].replace(/\\040/g, ' ').replace(/\\011/g, '\t');
    const fstype = parts[2];
    // 只认真实块设备（/dev/…），伪文件系统的 device 不以 /dev/ 开头，自然被排除
    if (!device.startsWith('/dev/')) continue;
    if (SKIP_FS.has(fstype)) continue;
    const prev = byDevice.get(device);
    if (prev === undefined || mountpoint.length < prev.length) {
      byDevice.set(device, mountpoint);
    }
  }
  const mountpoints = [...byDevice.values()];
  const sampled = await Promise.all(mountpoints.map((p) => sampleDiskAt(p)));
  const disks = sampled.filter((d): d is SystemMetrics['disk'] => d != null && d.totalBytes > 0);
  // 根卷排最前，其余按挂载路径排序
  disks.sort((a, b) => {
    if (a.path === '/') return -1;
    if (b.path === '/') return 1;
...
/**
 * macOS：项目所在根卷 + /Volumes 下挂载的其它卷（外接盘 / 数据盘）。
 * APFS 同容器会给出相同容量，按「总量+可用」签名去重。
 */
async function sampleMacDisks(rootPath: string): Promise<SystemMetrics['disk'][]> {
  const paths = new Set<string>(['/', rootPath]);
  try {
    for (const name of await fs.readdir('/Volumes')) paths.add(`/Volumes/${name}`);
  } catch {
    /* 没有 /Volumes 或读不到就只看根卷 */
  }
  const sampled = await Promise.all([...paths].map((p) => sampleDiskAt(p)));
  const disks = sampled.filter((d): d is SystemMetrics['disk'] => d != null && d.totalBytes > 0);
  const seen = new Set<string>();
  const unique: SystemMetrics['disk'][] = [];
  for (const d of disks) {
    const sig = `${d.totalBytes}:${d.freeBytes}`;
...
/**
 * 枚举机器上的所有卷（系统盘与数据盘一并计入）。
 *  - Windows：逐个盘符（C: … Z:）探测，保留可读出容量的卷。
 *  - Linux：读 /proc/mounts 枚举真实块设备卷。
 *  - macOS：根卷 + /Volumes 下的挂载卷。
 * 任一平台枚举为空时退回项目所在卷，避免误报。
 */
async function sampleAllDisks(platform: string, rootPath: string): Promise<SystemMetrics['disk'][]> {
  if (platform === 'win32') {
    const letters = 'CDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
    const results = await Promise.all(letters.map((l) => sampleDiskAt(`${l}:\\`)));
    const disks = results.filter((d): d is SystemMetrics['disk'] => d != null);
    // 盘符排序，C: 在前
    disks.sort((a, b) => a.path.localeCompare(b.path));
    if (disks.length > 0) return disks;
  } else if (platform === 'linux') {
    const disks = await sampleLinuxDisks();
    if (disks.length > 0) return disks;
  } else if (platform === 'darwin') {
    const disks = await sampleMacDisks(rootPath);
    if (disks.length > 0) return disks;
  }
  const root = await sampleDisk(rootPath);
  return [root];
}
...
export async function collectSystemMetrics(): Promise<SystemMetrics> {
  ...
  const disks = await sampleAllDisks(platform, PATHS.root);
