...
let linuxDistroCache: string | null | undefined;

/**
 * Linux 发行版名称。桌面 Linux 只靠 `os.release()` 得到的是内核版本（如 5.15.0-91-generic），
 * 看不出是 Ubuntu 还是 Debian。freedesktop 的 os-release 标准里 PRETTY_NAME 直接给出
 * 「Ubuntu 22.04.3 LTS」「Debian GNU/Linux 12 (bookworm)」这类可读名，主流发行版都提供。
 * 读不到（极简/裁剪系统）返回 null，由调用方退回内核版本。结果缓存一次。
 */
export function getLinuxDistroLabel(): string | null {
  if (linuxDistroCache !== undefined) return linuxDistroCache;
  linuxDistroCache = null;
  if (process.platform !== 'linux') return linuxDistroCache;

  const unquote = (v: string) => v.trim().replace(/^"(.*)"$/, '$1').replace(/^'(.*)'$/, '$1').trim();
  for (const file of ['/etc/os-release', '/usr/lib/os-release']) {
    const raw = readTextSync(file);
    if (!raw) continue;
    const pretty = /^PRETTY_NAME\s*=\s*(.+)$/im.exec(raw);
    if (pretty?.[1]) {
      const label = unquote(pretty[1]);
      if (label) { linuxDistroCache = label; break; }
    }
    // 没有 PRETTY_NAME 时用 NAME + VERSION 拼一个
    const name = /^NAME\s*=\s*(.+)$/im.exec(raw);
    const version = /^VERSION\s*=\s*(.+)$/im.exec(raw);
    if (name?.[1]) {
      linuxDistroCache = [unquote(name[1]), version ? unquote(version[1]) : '']
        .filter(Boolean).join(' ') || null;
      break;
    }
  }
  return linuxDistroCache;
}
...
export function getRuntimeLabel(): string {
  ...
  switch (kind) {
    ...
    case 'linux': {
      // 优先给发行版名（Ubuntu / Debian…），拿不到才退回内核版本，避免笼统的「Linux」
      const distro = getLinuxDistroLabel();
      return distro || `Linux ${release}`;
    }
    case 'termux': {
