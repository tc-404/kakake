...
/** 连接协议类型：OneBot、QQ 开放平台官方机器人、微信 iLink BOT */
export type ConnectionType = 'onebot' | 'qq_official' | 'weixin_bot';
...
export interface ConnectionConfig {
  ...
  /** QQ 官方：是否沙箱环境 */
  sandbox?: boolean;
  /** QQ 官方：Gateway Intents 位标志，缺省为常用组合 */
  intents?: number;
...
): conn is ConnectionConfig & { appId: string; appSecret: string } {
...
export function qqOfficialConnectionSummary(conn: ConnectionConfig): string {
  ...
  return `${kind} · AppID ${conn.appId} · ${env}`;
}
