...
export class OneBotHttpSseServer implements OneBotEndpoint {
  ...
  start(): void {
    ...
    this.server.on('listening', () => {
      ...
      if (!connection.accessToken && listenHostReachesBeyondLocalhost(host)) {
        logger.warn(
          `未配置 Access Token：${host}:${port} 的上报与事件流只对本机/内网开放，公网来源会被拒绝。`
  ...
  private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    if ((req.method === 'GET' || req.method === 'HEAD') && wantsSse) {
      const streamVerdict = checkHttpStreamAuth(req, connection.accessToken);
      if (!streamVerdict.ok) {
        logger.warn(`HTTP SSE 取流被拒绝：${streamVerdict.reason}`);
    ...
    try {
      ...
      if (!verdict.ok) {
        logger.warn(`HTTP SSE 上报被拒绝：${verdict.reason}`);
