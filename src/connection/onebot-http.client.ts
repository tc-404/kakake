...
export function createOnebotHttpClientWebhookMiddleware() {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const verdict = checkHttpEventAuth(req, handler.connection.accessToken, rawBody);
    if (!verdict.ok) {
      handler.logger.warn(`HTTP 客户端上报被拒绝：${verdict.reason}`);
...
export class OneBotHttpClient implements OneBotEndpoint {
  ...
  start(): void {
    ...
    if (!connection.accessToken) {
      logger.warn(
        '未配置 Access Token：该上报地址只接受本机/内网来源，公网来源会被拒绝。'
