...
export class QqOfficialWsClient implements QqOfficialEndpoint {
  ...
  /** 用户没显式配 intents 且被 4014 拒过 → 退到最小集合 */
  private fallbackIntentsUsed = false;
  ...
  start(): void {
    ...
    this.fallbackIntentsUsed = false;
  ...
  private async sendIdentify(): Promise<void> {
    ...
    this.log(
      ...
      + (this.fallbackIntentsUsed ? '（默认组合被拒后已自动降级）' : ''),
    );
    this.send({ op: 2, d });
  }

  /** 显式配置优先；未配置时用默认组合，被 4014 拒过后退到最小集合 */
  private resolveIntents(): number {
    ...
    return this.fallbackIntentsUsed ? MINIMAL_QQ_OFFICIAL_INTENTS : DEFAULT_QQ_OFFICIAL_INTENTS;
  }

  /** 用户显式配过 intents 就尊重配置；否则允许自动降级一次 */
  private mayDowngradeIntents(): boolean {
    const configured = Number(this.options.connection.intents);
    if (Number.isFinite(configured) && configured > 0) return false;
    return !this.fallbackIntentsUsed;
  }
  ...
  private handleClose(code?: number, reason?: string): void {
    ...
    if (code === 4014 && this.mayDowngradeIntents()) {
      this.fallbackIntentsUsed = true;
      // 必须清掉会话：否则下次连上会走 Resume（op6 不会改 intents），降级永远不生效
      this.sessionId = null;
      this.lastSeq = null;
      this.log(
        'warn',
        '[QQ官方] 平台以 4014（intents 未获授权）断开，已自动降级为「仅群聊/私聊消息」重试；'
        + '需要频道或互动事件时，请在连接设置里显式填写 intents',
      );
    }
