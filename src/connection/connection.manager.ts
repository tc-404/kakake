...
type ManagedEndpoint = OneBotEndpoint | QqOfficialEndpoint | WeixinBotEndpoint;

/**
 * 连接管理器 — OneBot / QQ 官方 / 微信 BOT
 */
export class ConnectionManager {
  ...
  getActiveEndpoint(): OneBotEndpoint | undefined {
    for (const ep of this.endpoints.values()) {
      if (ep instanceof QqOfficialWsClient || ep instanceof QqOfficialHttpsServer) continue;
  ...
  private persistQqOfficialBotProfile(connectionId: string): void {
    ...
    if (profile.username) conn.name = profile.username;
    configService.saveConnections(data);
  ...
  async refreshQqOfficialBotProfile(connectionId: string) {
    ...
    try {
      ...
      if (profile) {
        conn.botProfile = profile;
        conn.name = profile.username || conn.name;
        configService.saveConnections(data);
    ...
    const list = configService.getConnections().connections.map((c) => {
      ...
      if (isQqOfficialConnection(c)) {
        ...
        return {
          ...
          reconnectIntervalMs: c.reconnectIntervalMs ?? 5000,
          reconnectMaxAttempts: c.reconnectMaxAttempts ?? 15,
