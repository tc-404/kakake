...
export interface AppearanceState extends AppearanceSettings {
  ...
  /** 自定义标题名（手机置顶栏 / 电脑侧边栏顶部的品牌字），空串表示用默认「咔咔珂」 */
  customTitle: string;
}

/** 自定义标题名上限：20 个字符（按码点计，避免把 emoji 截断） */
export const CUSTOM_TITLE_MAX_LEN = 20;

/** 收口自定义标题：去首尾空白、按码点截到上限；非字符串或空 → 空串 */
...
export class AppearanceService {
  ...
  /** 读磁盘原始 JSON（含数值外观与 customTitle 等非数值字段），解析失败返回空对象 */
  private readRawFile(): Record<string, unknown> {
    if (!fs.existsSync(PATHS.appearance)) return {};
    try {
      const raw = JSON.parse(fs.readFileSync(PATHS.appearance, 'utf-8'));
  ...
  getCustomTitle(): string {
    return normalizeCustomTitle(this.readRawFile().customTitle);
  }

  /** 落盘：数值外观 + 保留（或覆盖）customTitle。空标题不写入该字段 */
  private writeFile(settings: AppearanceSettings, customTitle: string): void {
    this.ensureDir();
    const payload: Record<string, unknown> = { ...settings };
    if (customTitle) payload.customTitle = customTitle;
    fs.writeFileSync(PATHS.appearance, `${JSON.stringify(payload, null, 2)}\n`, 'utf-8');
  }

  saveSettings(raw: Partial<AppearanceSettings>): AppearanceSettings {
    const next = normalizeAppearance({ ...this.getSettings(), ...raw });
    // 保留已存的自定义标题，别被数值外观的保存冲掉
    this.writeFile(next, this.getCustomTitle());
    return next;
  }

  /** 单独保存自定义标题；返回收口后的结果（空串即恢复默认标题） */
  saveCustomTitle(raw: unknown): string {
    const title = normalizeCustomTitle(raw);
    this.writeFile(this.getSettings(), title);
    return title;
  }
