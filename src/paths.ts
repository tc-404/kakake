...
export const PATHS = {
  ...
  ssPluginsStatus: path.join(DATA_DIR, 'ss-plugins.json'),
  ...
  /** 远程公告更新状态（版本对比 / 待弹提示，非门禁） */
  announcementUpdate: path.join(DATA_DIR, 'announcement-update.json'),
  /** 在线更新：下载好的新版本包暂存目录（应用后清空；不可放业务数据） */
  updateStaging: path.join(DATA_DIR, 'update-staging'),
  /** 在线更新：待应用清单（apply-update.mjs 在两次进程之间读取） */
  updatePending: path.join(DATA_DIR, 'update-pending.json'),
  /** 资源商店来源开关（kakake / github）持久化 */
  pluginStoreState: path.join(DATA_DIR, 'plugin-store.json'),
  /** 资源商店列表本地缓存（跨重启秒开 + 后台刷新兜底） */
  pluginStoreCache: path.join(DATA_DIR, 'plugin-store-cache.json'),
  /** 临时数据目录：仅做「下载远程包 → 取版本号 → 即删」等一次性用途 */
