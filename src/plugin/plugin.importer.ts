...
import { isSsPluginDir, toSsPluginDirName } from './ss-plugin-id.js';

export type ImportPluginKind = 'kakake' | 'gf' | 'wx' | 'ss';
...
export class PluginImporter {
  ...
  private readonly ssLoader: PluginLoader;
  ...
  constructor(private readonly logger: Logger) {
    ...
    this.ssLoader = new PluginLoader(PATHS.plugins, PATHS.ssPluginsStatus, logger, isSsPluginDir);
  }
  ...
  async importFromZip(zipPath: string): Promise<ImportResult> {
    ...
    try {
      ...
      // 支持 zip / tar / tar.gz / rar / 7z / xz / bz2 等常见压缩包
      ...
      const validation = await loader.validatePluginEntry(entry.entryPath, entry.pluginPath);
      if (!validation.valid) {
        return {
          ok: false,
          pluginId,
      ...
      const kindLabel = kind === 'gf' ? '官方机器人' : kind === 'wx' ? '微信机器人' : kind === 'ss' ? '其他（KOOK）' : 'OneBot';
      return {
        ok: true,
        pluginId,
        kind,
  ...
  private detectKind(rawId: string, sourceDir: string): ImportPluginKind {
    ...
    if (
      isSsPluginDir(name)
      || isSsPluginDir(base)
      || /^ss[-_]?plugin/i.test(name)
      || /^ss[-_]plugin/i.test(base)
    ) {
      return 'ss';
    }
    try {
  ...
  private resolvePluginRoot(extractDir: string): { pluginId: string; sourceDir: string } {
    ...
    const hasEntry = (dir: string) => {
      ...
      for (const f of ['index.mjs', 'index.js', 'index.cjs', 'main.mjs', 'main.js', 'main.cjs']) {
