...
export class GfPluginManager {
  ...
  async open(): Promise<void> {
    ...
    this.enabled = true;
    this.logger.info('[GfPluginManager] 打开 GF 插件管理器（按账号多实例）...');
    await this.scanAndLoadPlugins();
    this.logger.info(`[GfPluginManager] 运行实例 ${this.runtimes.size} 个`);
  }
  ...
  private createEventContext(
    ...
    return {
      ...
      frameworkEnv: base.frameworkEnv
        ? { ...base.frameworkEnv, connectionId, ob11Mode: 'qq-official-ws' }
        : undefined,
    };
  }
  ...
  private async scanAndLoadPlugins(): Promise<void> {
    ...
    this.logger.info(`[GfPluginManager] 扫描到 ${this.plugins.size} 个 GF 插件`);
  ...
  /** 应加载该插件的全部账号（启用的 qq_official 连接 + 插件子开关 + 运行副本） */
