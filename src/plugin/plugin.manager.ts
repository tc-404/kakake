...
export class PluginManager implements IPluginManager {
  ...
  async open(): Promise<void> {
    ...
    this.enabled = true;
    this.logger.info('[PluginManager] 打开插件管理器（按账号多实例）...');
    await this.scanAndLoadPlugins();
    this.logger.info(`[PluginManager] 运行实例 ${this.runtimes.size} 个`);
  }
  ...
  private async scanAndLoadPlugins(): Promise<void> {
    ...
    this.logger.info(`[PluginManager] 扫描到 ${this.plugins.size} 个插件`);
