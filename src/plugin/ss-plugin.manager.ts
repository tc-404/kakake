...
const ADAPTER_NAME = 'ss_plugin_manager';
...
export class SsPluginManager {
  ...
  constructor(
    ...
    this.loader = new PluginLoader(PATHS.plugins, PATHS.ssPluginsStatus, logger, isSsPluginDir);
  }
  ...
  async open(): Promise<void> {
    ...
    this.enabled = true;
    this.logger.debug('[SsPluginManager] 打开 其他插件管理器（按账号多实例）...');
    await this.scanAndLoadPlugins();
    // 启动时只留一行；一个插件都没有时完全不打扰控制台
    const summary = `[SsPluginManager] 其他插件 ${this.plugins.size} 个 · 运行实例 ${this.runtimes.size} 个`;
  ...
  async close(): Promise<void> {
    ...
    this.logger.info('[SsPluginManager] 已关闭');
  }

  bindEvents(): void {
    eventBus.on('kook/event', (payload) => {
    ...
    eventBus.on('kook/action_result', (payload) => {
    ...
    eventBus.on('connection/ready', (payload) => {
      const p = payload as { connectionId?: string };
      if (!p.connectionId) return;
      if (pluginAccountService.ensureKookAccount(p.connectionId)) {
  ...
  async onEvent(
    ...
    if (!conn?.enable || (conn.type ?? 'onebot') !== 'kook') return;

    const accountKey = pluginAccountService.resolveAccountKey(conn);
    if (!accountKey) return;

    const enriched = { ...event, t: eventType ?? event.t, kook: true };
    ...
    for (const rt of this.runtimes.values()) {
      ...
      if (!connectionPluginService.isEnabled(connectionId, rt.pluginId, 'ss')) continue;
      targets.push(rt);
    }
  ...
  private async callPluginEventHandler(
    ...
    try {
      ...
      this.logger.error(
        `[SsPluginManager] 插件 ${entry.id}@${rt.accountKey} 事件错误 (${connectionId}):`,
        error,
      );
    }
  }
  ...
  private createEventContext(
    ...
    return {
      ...
      configPath,
      actions: {
        call: this.connectionManager.createKookBotActionCaller(adapterName, connectionId),
      },
      frameworkEnv: base.frameworkEnv
        ? { ...base.frameworkEnv, connectionId, ob11Mode: 'kook-gateway' }
        : undefined,
    };
  }
  ...
  private async scanAndLoadPlugins(): Promise<void> {
    ...
    this.logger.debug(`[SsPluginManager] 扫描到 ${this.plugins.size} 个 其他插件`);
  ...
  listTargetAccountKeys(pluginId: string): string[] {
    ...
    for (const conn of configService.getConnections().connections) {
      if (!conn.enable || (conn.type ?? 'onebot') !== 'kook') continue;
      if (!connectionPluginService.isEnabled(conn.id, pluginId, 'ss')) continue;
  ...
  getLoadedAccountKeys(pluginId: string): string[] {
    const id = resolveSsPluginId(pluginId);
    const keys: string[] = [];
    for (const rt of this.runtimes.values()) {
  ...
  isLoadedForAccount(pluginId: string, accountKey: string): boolean {
    return this.runtimes.has(pluginRuntimeKey(accountKey, resolveSsPluginId(pluginId)));
  }
  ...
  private prepareRuntimePaths(entry: PluginEntry, accountKey: string): boolean {
    try {
      ...
      this.logger.error(`[SsPluginManager] 准备运行副本失败 ${entry.id}@${accountKey}:`, e);
      return false;
    }
  }
  ...
  private async loadRuntime(pluginId: string, accountKey: string): Promise<boolean> {
    ...
    if (!module) {
      this.logger.error(`[SsPluginManager] 模块加载失败 ${pluginId}@${accountKey}`);
      return false;
    }
    ...
    try {
      ...
      this.logger.info(
        `[SsPluginManager] 初始化: ${entry.id}${entry.version ? ` v${entry.version}` : ''} @${accountKey}`,
      );
      ...
      this.logger.error(`[SsPluginManager] 初始化失败 ${entry.id}@${accountKey}:`, error);
      return false;
    }
  }
  ...
  private async unloadRuntime(runtimeKey: string): Promise<void> {
    ...
    if (entry.runtime.status === 'loaded') {
      ...
      if (module && context && typeof module.plugin_cleanup === 'function') {
        try {
          ...
          this.logger.error(`[SsPluginManager] 清理失败 ${pluginId}@${accountKey}:`, error);
        }
      }
    }
    ...
    this.logger.info(`[SsPluginManager] 已卸载: ${pluginId}@${accountKey}（运行实例 ${this.runtimes.size}）`);
  ...
  private createPluginContext(
    ...
    const actions = {
      call: this.connectionManager.createKookBotActionCaller(
    ...
    const getPluginExports = <T = PluginModule>(pluginId: string): T | undefined => {
      const prefer = this.runtimes.get(pluginRuntimeKey(accountKey, resolveSsPluginId(pluginId)));
      const rt = prefer ?? [...this.runtimes.values()].find((r) => r.pluginId === resolveSsPluginId(pluginId));
  ...
  getRuntimeEntry(pluginId: string, accountKey: string): PluginEntry | undefined {
    const entry = this.resolvePluginEntry(pluginId);
    const id = entry?.id ?? resolveSsPluginId(pluginId);
  ...
  private resolvePluginEntry(pluginId: string): PluginEntry | undefined {
    ...
    const resolved = resolveSsPluginId(trimmed);
  ...
  async rescanPlugins(): Promise<number> {
    ...
    for (const [id] of Array.from(this.plugins.entries())) {
      if (!scannedMap.has(id)) {
        ...
        this.logger.info(`[SsPluginManager] 插件目录已移除: ${id}`);
      }
    }
    ...
    this.logger.info(`[SsPluginManager] 重新扫描完成，共 ${this.plugins.size} 个插件 / ${this.runtimes.size} 个实例`);
    return this.plugins.size;
  }

  async setConnectionPluginStatus(connectionId: string, pluginId: string, enable: boolean): Promise<void> {
    const resolvedId = resolveSsPluginId(pluginId);
    if (enable) {
      ...
      const accountKey = pluginAccountService.resolveAccountKey(conn);
      if (!accountKey) {
        throw new Error('KOOK 账号未就绪：请先启用连接，连上网关获取机器人身份后再启用插件');
      }
      pluginAccountService.ensureAccountRuntimeRoot(accountKey);
      pluginAccountService.copyPluginToAccount(resolvedId, accountKey);
    }
    connectionPluginService.setEnabled(connectionId, resolvedId, enable, 'ss');
  ...
  async registerImportedPlugin(pluginId: string): Promise<boolean> {
    ...
    if (!entry?.entryPath) {
      this.logger.warn(`[SsPluginManager] 导入后无法扫描插件: ${pluginId}`);
      return false;
    }
    this.plugins.set(entry.id, entry);
  ...
  isMasterEnabled(pluginId: string): boolean {
    return this.loader.isMasterEnabled(resolveSsPluginId(pluginId));
  }
  ...
  async uninstallPlugin(pluginId: string, cleanData = false): Promise<void> {
    ...
    connectionPluginService.removePluginEverywhere(entry.id, 'ss');
  ...
  async removeConnectionRuntimePlugin(
    ...
    const accountKey = pluginAccountService.resolveAccountKey(conn);
    connectionPluginService.setEnabled(connectionId, resolvedId, false, 'ss');
    await this.syncPluginRuntime(resolvedId);
    if (accountKey) {
