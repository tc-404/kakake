...
export class WxPluginManager {
  ...
  async open(): Promise<void> {
    ...
    this.enabled = true;
    this.logger.debug('[WxPluginManager] 打开 微信插件管理器（按账号多实例）...');
    await this.scanAndLoadPlugins();
    // 启动时只留一行；一个插件都没有时完全不打扰控制台
    const summary = `[WxPluginManager] 微信插件 ${this.plugins.size} 个 · 运行实例 ${this.runtimes.size} 个`;
  ...
  private async scanAndLoadPlugins(): Promise<void> {
    ...
    this.logger.debug(`[WxPluginManager] 扫描到 ${this.plugins.size} 个 微信插件`);
