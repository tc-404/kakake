...
# ---------- 受管重启循环 ----------
# 后端可能以退出码 86 退出，表示「请应用暂存更新并重启」。设置该变量让控制台点亮
# 「确定重启」按钮；本脚本据退出码 86 循环：应用更新（scripts/apply-update.mjs）后重新拉起。
...
run_kakake_loop() {
  while :; do
    ...
    set -e
    if [ "$status" = "86" ]; then
      echo "[Kakake] 应用更新 / 重启中…"
      node scripts/apply-update.mjs || true
      # 类 Unix 可直接覆盖运行中可执行文件的 inode，无需 Windows 那种 .new 交换
      continue
    fi
    return "$status"
  done
