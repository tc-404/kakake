...
【重要声明 · 关于更新】
  - 本免 Node.js 便携版为「一次性产品」，不支持在线更新，只能手动部署更新：
    下载新版便携包 → 解压 → 覆盖到原目录（保留 data/、log/、plugins/、plugins_two/ 数据目录即可）。
  - 只有开发版（源码版）才支持控制台内一键在线更新。
  - 控制台内若出现更新入口，对便携版一律以本声明为准，请勿用于便携版自更新。
