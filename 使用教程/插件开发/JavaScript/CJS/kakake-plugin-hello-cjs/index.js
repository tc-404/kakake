// ========== 加载时 ==========
// module.exports = 把东西交出去给框架（CommonJS 的写法，对应 ESM 的 export）
// 下面最后一行把 plugin_init / plugin_onmessage 一起交出去，框架靠名字找到它们
// async = 函数里可能要 await 等待
// ctx = 上下文（工具箱）：logger、actions 等都在里面
async function plugin_init(ctx) {
  // logger.info = 往日志里打一行说明文字
  ctx.logger.info('你好示例插件（CJS 版）已加载');
}
...
// ========== 交给框架 ==========
// 必须写成一个**字面量对象**，名字要一眼能看出来（框架靠名字认钩子）
// 不要写成 module.exports = api 这种运行时拼装的形式（虽然宿主兼容层也认，但没必要冒险）
module.exports = {
  plugin_init,
  plugin_onmessage,
};
