#!/bin/bash
# mk - Linux server management tool

set -euo pipefail

readonly MK_VERSION="2.5.6"
readonly INSTALL_DIR="/usr/local/lib/mk-tools"
readonly INSTALL_BIN="/usr/local/bin/mk"
...
require_root() {
    if [[ $EUID -ne 0 ]]; then
        error "此操作需要 root 权限，请使用: sudo mk"
        return 1
    fi
}
...
install_self() {
    ...
    for bin_name in "${INSTALL_BIN_ALIASES[@]}"; do
        ln -sf "$installed" "/usr/local/bin/${bin_name}"
    done
    mk_embed_extract_patches "${INSTALL_DIR}/napcat-patches" || warn "NapCat 魔改资源解压失败"
...
EOF
    ...
    info "mk 工具已安装/更新完成 版本: ${MK_VERSION}"
    info "安装路径: ${INSTALL_DIR}/${SCRIPT_FILE}"
    info "全局命令: mk / MK / Mk / mK（大小写均可）"
}
...
uninstall_self() {
    ...
    local confirm=""
    read -r -p "确认卸载 mk 工具? [y/N]: " confirm
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        info "已取消卸载"
        return 0
    fi
...
auto_install_if_needed() {
    ...
    if [[ "$SELF_PATH" != "$installed" ]]; then
        if [[ ! -f "$SELF_PATH" ]]; then
            ...
            error "找不到 mk 脚本: ${SELF_PATH}"
            exit 1
        fi
        if [[ $EUID -ne 0 ]]; then
            error "首次安装需要 root 权限"
            echo "请执行: sudo bash $(basename "$SELF_PATH")"
            exit 1
        fi
        title "MK 工具 - 安装/更新"
        install_self || exit 1
        info "现在可以直接输入 mk / MK 使用菜单"
    fi
}
...
baota_watch_install() {
    ...
    echo
    info "正在监听宝塔安装进度，进程号=${pid}"
    info "日志文件: ${BAOTA_LOG}"
    info "按 Ctrl+C 可退出监听，后台安装不会停止"
    echo

    trap 'echo; info "已退出监听，安装仍在后台继续"; trap - INT; return 0' INT
    ...
    echo
    warn "安装进程已结束，请查看日志确认结果"
    info "日志: tail -f ${BAOTA_LOG}"
    return 0
}
...
menu_baota_select_stable() {
    while true; do
        title "宝塔稳定版"
        show_nav_hint
        echo "  [2] 9.0 LTS 长期支持版"
        echo "  [3] 10.0 LTS 长期支持版"
        echo
...
menu_baota_select_version() {
    while true; do
        title "选择宝塔安装版本"
        show_nav_hint
        echo "  [2] 正式版 (最新 11.x，推荐)"
        echo "  [3] 稳定版 (9.0 / 10.0 LTS)"
        echo
...
baota_do_install() {
    ...
    info "正在后台启动宝塔安装: ${BAOTA_INSTALL_LABEL}"
...
baota_show_password() {
    ...
    title "宝塔面板登录信息"
    echo "  面板地址: ${panel_url:-获取失败}"
    if [[ -n "$panel_inner" && "$panel_inner" != "$panel_url" ]]; then
        echo "  内网地址: ${panel_inner}"
    fi
    echo "  用户名:   ${username:-获取失败}"
    echo "  密码:     ${password:-获取失败}"
    echo
    warn "若无法访问，请在安全组放行面板端口"
}

show_nav_hint() {
    echo -e "${YELLOW}  [0] 返回首页    [1] 返回上一步${NC}"
    echo -e "${YELLOW}  --------------------------------${NC}"
}
...
menu_baota() {
    ...
    while true; do
        ...
        if baota_install_running; then
            echo "  [4] 查看安装进度"
            echo
            echo -e "  ${YELLOW}* 宝塔正在后台安装中...${NC}"
        elif is_baota_installed; then
            echo
            echo -e "  ${GREEN}* 宝塔已安装${NC}"
        fi
        echo
...
napcat_firewall_open_port() {
    ...
    if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi active; then
        ...
        info "已通过 ufw 放行 ${port} 端口"
    elif command -v firewall-cmd >/dev/null 2>&1; then
        firewall-cmd --permanent --add-port="${port}/tcp" >/dev/null 2>&1 || true
        firewall-cmd --reload >/dev/null 2>&1 || true
        info "已通过 firewalld 放行 ${port} 端口"
    elif command -v iptables >/dev/null 2>&1; then
        iptables -C INPUT -p tcp --dport "$port" -j ACCEPT >/dev/null 2>&1 || \
            iptables -I INPUT -p tcp --dport "$port" -j ACCEPT >/dev/null 2>&1 || true
        info "已通过 iptables 放行 ${port} 端口"
    else
        warn "未检测到常见防火墙，请手动放行 ${port} 端口"
    fi
    warn "请在云服务商安全组放行 ${port} 端口"
}
...
napcat_firewall_close_port() {
    ...
    if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi active; then
        ...
        info "已通过 ufw 关闭 ${port} 端口"
    elif command -v firewall-cmd >/dev/null 2>&1; then
        firewall-cmd --permanent --remove-port="${port}/tcp" >/dev/null 2>&1 || true
        firewall-cmd --reload >/dev/null 2>&1 || true
        info "已通过 firewalld 关闭 ${port} 端口"
    elif command -v iptables >/dev/null 2>&1; then
        ...
        info "已通过 iptables 关闭 ${port} 端口"
    else
        warn "未检测到常见防火墙，请手动关闭 ${port} 端口"
    fi
}
...
napcat_show_login_info() {
    ...
    title "NapCat WebUI 登录信息"
    echo "  面板地址: ${url}"
    echo "  登录密钥: ${token:-获取失败，请先启动框架或查看日志}"
    if [[ -f "$NAPCAT_VER_FILE" ]]; then
        echo "  安装版本: $(cat "$NAPCAT_VER_FILE" 2>/dev/null || echo 未知)"
    fi
    echo
    warn "若无法访问，请在安全组放行 ${port} 端口"
}
...
napcat_watch_install() {
    ...
    echo
    info "正在监听 NapCat 安装进度，进程号=${pid}"
    info "日志文件: ${NAPCAT_INSTALL_LOG}"
    info "按 Ctrl+C 可退出监听，后台安装不会停止"
    echo
    ...
    if napcat_is_installed || [[ "$state" == "100" ]]; then
        ...
        echo
        info "NapCat 安装成功，已开放 6099 端口"
        info "未自动启动，请使用菜单「启动框架」手动启动"
        napcat_clear_install_state
        return 0
    fi
    ...
    echo
    warn "安装进程已结束，请查看日志确认结果"
    info "日志: tail -f ${NAPCAT_INSTALL_LOG}"
    return 0
}

menu_napcat_select_version() {
    while true; do
        title "选择 NapCat 安装版本"
        show_nav_hint
        echo "  [2] 最新版 (官方一键脚本)"
        echo "  [3] v4.18.5 (插件最后支持)"
        echo "  [4] v4.17.23 (支持上传插件)"
        echo "  [5] v4.15.0"
        echo
            ...
            *)
                if [[ "$choice" =~ ^[0-9]+$ ]]; then
                    ...
                    if (( idx >= 0 && idx < ${#napcat_mirror_labels[@]} )); then
                        ...
                        info "已选择: ${napcat_mirror_labels[$idx]}"
                        return 0
                    fi
                fi
                warn "无效选项，请重新输入"
...
napcat_do_install() {
    ...
    info "正在后台启动 NapCat 安装: ${NAPCAT_VERSION_LABEL}"
...
napcat_start() {
    ...
    napcat_open_port_6099
    info "NapCat 启动成功 (后台运行)"
    info "退出 mk 菜单不会停止 NapCat，请使用「停止框架」关闭"
    napcat_show_login_info
}
...
napcat_show_logs() {
    if [[ ! -f "$NAPCAT_LOG" ]]; then
        warn "暂无运行日志"
        if [[ -f "$NAPCAT_INSTALL_LOG" ]]; then
            info "可查看安装日志: ${NAPCAT_INSTALL_LOG}"
        fi
        return 1
    fi

    title "NapCat 运行日志 (最近 80 行)"
    tail -n 80 "$NAPCAT_LOG"
    echo
    echo "  完整日志: ${NAPCAT_LOG}"
    echo "  实时跟踪: tail -f ${NAPCAT_LOG}"
    echo
    info "退出菜单不会停止 NapCat 后台进程"
}

napcat_do_uninstall() {
    title "卸载 NapCat 框架"
    echo "  将彻底删除 NapCat 目录、安装脚本及运行状态"
    echo "  包括: ~/Napcat、napcat.sh、NapCat.Shell.zip 等"
    echo

    local confirm=""
    read -r -p "确认彻底卸载 NapCat? [y/N]: " confirm
    [[ "$confirm" =~ ^[Yy]$ ]] || {
        info "已取消卸载"
        return 0
    }
...
napcat_cfg_delete_item() {
    ...
    if [[ -n "$del_port" && "$del_port" =~ ^[0-9]+$ && "$del_port" != "0" ]]; then
        if napcat_cfg_port_used_by_others "$del_port"; then
            info "端口 ${del_port} 仍被其他配置使用，防火墙规则保留"
        else
...
menu_napcat_net_add() {
    ...
    while true; do
        ...
        echo "  [2] HTTP 服务端"
        echo "  [3] HTTP 客户端"
        echo "  [4] WebSocket Server (正向 WS，NapCat 监听)"
        echo "  [5] WebSocket Client (反向 WS，NapCat 连出)"
        echo
    ...
    echo
    read -r -p "配置名称 (唯一标识): " name
    name="${name// /}"
    [[ -z "$name" ]] && name="NetCfg_$(date +%s)"

    if [[ "$cat" == "httpServers" || "$cat" == "websocketServers" ]]; then
        read -r -p "监听地址 [0.0.0.0]: " host
        host="${host:-0.0.0.0}"
        if [[ "$cat" == "httpServers" ]]; then
            read -r -p "监听端口 [3000]: " port
            port="${port:-3000}"
        else
            read -r -p "监听端口 [3001]: " port
            port="${port:-3001}"
        fi
        read -r -p "鉴权 Token (留空跳过): " token
        read -r -p "是否启用? [Y/n]: " enable
        ...
        if [[ "$cat" == "httpClients" ]]; then
            read -r -p "上报地址 [http://127.0.0.1:8080]: " url
            url="${url:-http://127.0.0.1:8080}"
        else
            read -r -p "连接地址 [ws://127.0.0.1:8082]: " url
    ...
    info "配置已保存: ${cfg_path:-onebot11.json}"
    info "网络配置已立即生效，无需重启 NapCat"
}
...
menu_napcat() {
    if napcat_install_running; then
        info "检测到 NapCat 正在后台安装，正在恢复进度显示"
        napcat_watch_install
        echo
    fi
    ...
    while true; do
        ...
        echo "  [7] 卸载框架"
        echo "  [8] 查看日志"
        echo "  [9] 查看NapCat面板登录密钥"
        if napcat_install_running; then
            echo "  [10] 查看安装进度"
            echo
            echo -e "  ${YELLOW}* NapCat 正在后台安装中...${NC}"
        elif napcat_is_running; then
            echo
            echo -e "  ${GREEN}* NapCat 正在后台运行${NC}"
        elif napcat_is_installed; then
            echo
            echo -e "  ${YELLOW}* NapCat 已安装 (未运行)${NC}"
        else
            echo
            echo -e "  ${YELLOW}* NapCat 未安装${NC}"
        fi
        echo
...
menu_snowluma_vnc_config() {
    ...
    title "配置扫码桌面端口"
    show_nav_hint
    echo "  当前 noVNC 端口: ${SNOWLUMA_NOVNC_PORT}  (浏览器扫码，默认 6081，可改)"
    echo "  当前 VNC 端口:   ${SNOWLUMA_VNC_PORT}  (客户端，默认 5900，可改)"
    echo "  当前 VNC 密码:   ${SNOWLUMA_VNC_PASS}"
    echo "  配置文件: ${SNOWLUMA_VNC_CFG}"
    echo
    echo "  直接回车保留原值；改完后需重启扫码桌面/QQ 才生效"
    echo
    read -r -p "noVNC 端口 [${SNOWLUMA_NOVNC_PORT}]: " np
    read -r -p "VNC 端口 [${SNOWLUMA_VNC_PORT}]: " vp
    read -r -p "VNC 密码 [${SNOWLUMA_VNC_PASS}]: " pw
    ...
    if [[ "$SNOWLUMA_NOVNC_PORT" == "$SNOWLUMA_VNC_PORT" ]]; then
        error "noVNC 与 VNC 端口不能相同"
        return 1
    fi
    snowluma_vnc_save_cfg
    info "已保存: noVNC=${SNOWLUMA_NOVNC_PORT}  VNC=${SNOWLUMA_VNC_PORT}"
    if snowluma_vnc_is_running || snowluma_qq_is_running; then
        local restart=""
        read -r -p "扫码桌面/QQ 正在运行，是否立即按新端口重启? [y/N]: " restart
        if [[ "$restart" =~ ^[Yy]$ ]]; then
            local qq_was=0
            ...
            *)
                if [[ "$choice" =~ ^[0-9]+$ ]]; then
                    ...
                    if (( idx >= 0 && idx < ${#snowluma_mirror_labels[@]} )); then
                        ...
                        info "已选择: ${snowluma_mirror_labels[$idx]}"
                        return 0
                    fi
                fi
                warn "无效选项，请重新输入"
...
menu_snowluma_select_version() {
    ...
    while true; do
        title "选择 SnowLuma 版本"
        show_nav_hint
        echo "  [2] 最新版 (GitHub Releases latest)"
        echo "  [3] 手动输入版本号 (如 v1.13.0)"
        echo
        local choice="" tag=""
        read_choice choice
        case "$choice" in
...
menu_snowluma_select_mode() {
    ...
    while true; do
        title "选择 SnowLuma 安装模式"
        show_nav_hint
        echo "  [2] Docker 安装 (推荐 · 内置 QQ + VNC/noVNC)"
        echo "  [3] 直接安装 (解压发行包到 ${SNOWLUMA_HOME})"
        echo
        echo "  Docker：镜像 ${SNOWLUMA_DOCKER_IMAGE}"
        echo "  直接安装：需本机自行装兼容 QQ，WebUI 默认 ${SNOWLUMA_WEBUI_PORT}"
        echo
        local choice=""
        read_choice choice
        case "$choice" in
...
snowluma_native_asset_name() {
    ...
    case "$arch" in
        ...
        *)
            error "不支持的架构: ${arch}（仅 amd64/arm64）"
            return 1
            ;;
    esac
}
...
snowluma_install_docker() {
    if ! command -v docker >/dev/null 2>&1; then
        error "未检测到 docker，请先安装 Docker 后再选 Docker 模式"
        return 1
    fi

    if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx "$SNOWLUMA_DOCKER_NAME"; then
        local re=""
        warn "已存在同名容器: ${SNOWLUMA_DOCKER_NAME}"
        read -r -p "是否删除并重建? [y/N]: " re
        [[ "$re" =~ ^[Yy]$ ]] || return 0
        docker rm -f "$SNOWLUMA_DOCKER_NAME" >/dev/null 2>&1 || true
    fi

    info "拉取镜像 ${SNOWLUMA_DOCKER_IMAGE} ..."
...
snowluma_install_native() {
    ...
    if [[ -f "${tmpdir}/launcher.sh" ]]; then
        ...
        if [[ -n "$inner" ]]; then
            ...
            if [[ -z "$inner" ]]; then
                error "解压后未找到 launcher.sh / index.mjs"
                rm -rf "$tmpdir"
                return 1
            fi
            cp -a "$(dirname "$inner")/." "$SNOWLUMA_HOME/"
    ...
    info "直接安装完成: ${tag} → ${SNOWLUMA_HOME}"
    info "请先「安装 QQ」，再「启动 SnowLuma」"
}
...
snowluma_do_install() {
    ...
    if snowluma_is_installed; then
        local re=""
        warn "检测到 SnowLuma 已安装 (模式: $(snowluma_get_mode))"
        read -r -p "是否重新安装? [y/N]: " re
        [[ "$re" =~ ^[Yy]$ ]] || return 0
        snowluma_stop || true
    fi
...
snowluma_do_uninstall() {
    ...
    mode="$(snowluma_get_mode)"
    echo "  当前模式: ${mode:-未安装}"
    echo "  Docker：删除容器（默认保留数据卷）"
    echo "  直接安装：删除 ${SNOWLUMA_HOME}"
    echo
    local confirm=""
    read -r -p "确认卸载 SnowLuma? [y/N]: " confirm
    ...
    if [[ "$mode" == "docker" ]] || docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx "$SNOWLUMA_DOCKER_NAME"; then
        docker rm -f "$SNOWLUMA_DOCKER_NAME" >/dev/null 2>&1 || true
        local wipe=""
        read -r -p "是否同时删除 Docker 数据卷 (扫码登录会丢失)? [y/N]: " wipe
        if [[ "$wipe" =~ ^[Yy]$ ]]; then
...
menu_snowluma_install_qq() {
    ...
    info "正在刷新 QQ 下载列表（官网 rainbow + 内置兼容源）..."
    snowluma_list_qq_options

    while true; do
        title "安装 QQ（SnowLuma 兼容列表）"
        show_nav_hint
        echo "  系统架构: ${arch}    包格式偏好: ${pkg}"
        echo "  SnowLuma 版本: ${snow_ver:-未安装}    模式: ${mode:-无}"
        echo "  下载域名: 优先 dldir1v6.qq.com（官网新域名），失败自动试 GitHub 归档"
        echo "  官网: https://im.qq.com/linuxqq/index.shtml"
        echo
        if [[ "$mode" == "docker" ]]; then
            echo -e "  ${YELLOW}* Docker 模式镜像已内置 QQ，一般无需再装宿主 QQ${NC}"
            echo -e "  ${YELLOW}* 下列地址供对照/备用；选安装将装到宿主机 /opt/QQ${NC}"
            echo
        fi

        if ((${#SNOWLUMA_QQ_MATCH_LABELS[@]} == 0)); then
            warn "当前架构暂无可用直链，请到官网下载对应 ${arch} 包"
            echo
        else
            for ((i=0; i<${#SNOWLUMA_QQ_MATCH_LABELS[@]}; i++)); do
                idx=$((i + 2))
                echo "  [${idx}] 安装 ${SNOWLUMA_QQ_MATCH_LABELS[$i]}"
                local u
...
snowluma_qq_install_from_urlset() {
    ...
    if [[ -x "/opt/QQ/qq" ]]; then
        local re=""
        warn "检测到已安装 /opt/QQ/qq"
        read -r -p "是否覆盖安装 ${ver}? [y/N]: " re
        [[ "$re" =~ ^[Yy]$ ]] || return 0
        snowluma_qq_stop || true
    fi
    ...
    if [[ -z "$ok_url" ]]; then
        error "所有下载源均不可用（含官网 dldir1v6 与归档镜像）"
        rm -rf "$tmp"
        return 1
    fi
    ...
    if [[ "${sz:-0}" -lt 1000000 ]]; then
        error "下载文件异常偏小 (${sz} bytes)，可能仍是 404 页"
        rm -rf "$tmp"
        return 1
    fi
    ...
    if [[ "$pkgfile" == *.rpm ]]; then
        ...
        if command -v apt-get >/dev/null 2>&1; then
            ...
            error "当前系统无 dpkg/apt，无法安装 deb 包"
            rm -rf "$tmp"
            return 1
        fi
    fi
    ...
    rm -rf "$tmp"
    if [[ -x "/opt/QQ/qq" ]]; then
        info "QQ ${ver} 安装完成: /opt/QQ/qq"
        snowluma_qq_ensure_runtime_deps || true
    else
        warn "安装命令已执行，但未检测到 /opt/QQ/qq，请检查输出"
        return 1
    fi
}
...
snowluma_vnc_ensure_deps() {
    ...
    require_root || return 1
    warn "缺少扫码桌面依赖，正在安装 Xvfb / x11vnc / 轻量窗口管理器 ..."
    if command -v apt-get >/dev/null 2>&1; then
        export DEBIAN_FRONTEND=noninteractive
        apt-get update -qq >/dev/null 2>&1 || true
    ...
    command -v Xvfb >/dev/null 2>&1 || { error "Xvfb 安装失败"; return 1; }
    command -v x11vnc >/dev/null 2>&1 || { error "x11vnc 安装失败"; return 1; }
    command -v openbox >/dev/null 2>&1 || command -v fluxbox >/dev/null 2>&1 \
        || { error "openbox/fluxbox 安装失败"; return 1; }
    info "扫码桌面依赖已就绪"
}
...
snowluma_vnc_ensure_novnc() {
    ...
    command -v git >/dev/null 2>&1 || { error "需要 git 以下载 noVNC"; return 1; }
    info "下载 noVNC 到 ${SNOWLUMA_NOVNC_DIR} ..."
...
snowluma_vnc_start_stack() {
    ...
    snowluma_vnc_ensure_index "$webroot" || warn "未能写入 noVNC index.html，请改用 /vnc.html"

    # QQ 已在虚拟屏上跑时：只补远程查看，绝不重启 Xvfb（否则会杀掉 QQ）
    if snowluma_xvfb_is_up; then
        if snowluma_qq_is_running && snowluma_qq_on_mk_display; then
            info "检测到 QQ 已在 ${SNOWLUMA_DISPLAY} 运行，仅拉起/保持远程查看（不重启虚拟屏）"
        ...
        if snowluma_vnc_is_running; then
            ...
            info "若根路径是目录列表，请打开: http://$(napcat_get_server_ip):${SNOWLUMA_NOVNC_PORT}/vnc.html"
            return 0
        fi
        snowluma_vnc_start_viewer "$webroot" || return 1
        if ! snowluma_qq_is_running; then
            warn "当前还没启动 QQ：桌面可能是灰/黑空屏，请回菜单点「启动 QQ」"
        fi
        return 0
    fi
    ...
    if snowluma_qq_is_running; then
        if snowluma_qq_on_mk_display; then
            ...
            warn "虚拟屏 ${SNOWLUMA_DISPLAY} 已丢失，但 QQ 仍占着该 DISPLAY，将结束 QQ 后重建扫码桌面"
            snowluma_qq_kill_all || true
            sleep 1
        else
            error "检测到脱管 QQ（未挂扫码屏 ${SNOWLUMA_DISPLAY}），且虚拟屏未运行"
            error "请先点「彻底杀死 QQ」或「启动 QQ」（启动会自动清掉脱管进程再拉起）"
            return 1
        fi
    fi
    ...
    if [[ ! -S "/tmp/.X11-unix/X${SNOWLUMA_DISPLAY_NUM}" ]]; then
        error "Xvfb 启动失败，见 ${SNOWLUMA_VNC_LOG}"
        return 1
    fi
...
snowluma_vnc_start_viewer() {
    ...
    if ! snowluma_vnc_is_running; then
        info "启动 x11vnc :${SNOWLUMA_VNC_PORT} ..."
        ...
        sleep 0.5
        if ! snowluma_vnc_is_running; then
            error "x11vnc 启动失败，见 ${SNOWLUMA_VNC_LOG}"
            return 1
        fi
    fi

    if ! snowluma_pidfile_running "$SNOWLUMA_NOVNC_PID"; then
        info "启动 noVNC :${SNOWLUMA_NOVNC_PORT} ..."
        if [[ -x "${webroot}/utils/novnc_proxy" ]]; then
            ...
            error "未找到 websockify / novnc_proxy"
            return 1
        fi
        sleep 0.5
    fi
    ...
    snowluma_open_ports "$SNOWLUMA_VNC_PORT" "$SNOWLUMA_NOVNC_PORT"
    info "远程查看已就绪（QQ/虚拟屏保持不动）"
    info "noVNC: http://$(napcat_get_server_ip):${SNOWLUMA_NOVNC_PORT}/vnc.html"
    info "VNC:   $(napcat_get_server_ip):${SNOWLUMA_VNC_PORT}  密码: ${SNOWLUMA_VNC_PASS}"
}
...
snowluma_vnc_stop_viewer() {
    ...
    info "已停止远程查看 (noVNC/VNC)，QQ 与虚拟屏 ${SNOWLUMA_DISPLAY} 仍在运行"
    if snowluma_qq_is_running; then
        info "QQ 进程未受影响"
    fi
}
...
snowluma_show_vnc_info() {
    ...
    snowluma_vnc_load_cfg
    host_ip="$(napcat_get_server_ip)"
    title "SnowLuma 扫码桌面 (VNC / noVNC)"
    echo "  用途:     浏览器打开 noVNC，在 QQ 窗口里扫码登录"
    echo "  显示:     ${SNOWLUMA_DISPLAY} (虚拟屏，非完整桌面)"
    if snowluma_vnc_is_running || [[ "$(snowluma_get_mode)" == "docker" ]]; then
        echo -e "  状态:     ${GREEN}运行中${NC}"
    elif snowluma_xvfb_is_up; then
        echo -e "  状态:     ${YELLOW}虚拟屏在、远程查看未开${NC}"
    else
        echo -e "  状态:     ${YELLOW}未运行${NC}"
    fi
    if snowluma_qq_is_running && ! snowluma_qq_on_mk_display; then
        echo -e "  警告:     ${YELLOW}QQ 脱管（未挂 ${SNOWLUMA_DISPLAY}），扫码无效；请「启动 QQ」自动修复${NC}"
    fi
    ...
    echo "  密码:     ${SNOWLUMA_VNC_PASS}"
    echo "  改端口:   菜单「配置扫码端口」或编辑 ${SNOWLUMA_VNC_CFG}"
    if [[ "$(snowluma_get_mode)" == "docker" ]]; then
        echo "  说明:     Docker 模式由容器映射提供（改端口需改 docker -p）"
    else
        echo "  日志:     ${SNOWLUMA_VNC_LOG}"
        echo "  提示:     「启动 QQ」会自动拉起扫码桌面"
        echo "  注意:     若根路径显示目录列表，请点开 /vnc.html"
        echo "  登完后:   可「仅停止远程查看」，QQ 继续跑；关浏览器即可不扫码"
    fi
    echo
}
...
snowluma_qq_ensure_runtime_deps() {
    ...
    if ! ldconfig -p 2>/dev/null | grep -q 'libasound\.so\.2'; then
        error "仍缺少 libasound.so.2，请手动安装: apt install libasound2 或 libasound2t64"
        return 1
    fi
...
snowluma_qq_disable_autostart_hints() {
    ...
    if command -v systemctl >/dev/null 2>&1; then
        if systemctl is-enabled "$AUTOSTART_NAPCAT_UNIT" >/dev/null 2>&1; then
            warn "检测到 ${AUTOSTART_NAPCAT_UNIT} 已启用：它也可能拉起 /opt/QQ/qq"
            warn "若 QQ 仍反复自启，请到主菜单「开机自启」关闭 NapCat 自启"
        fi
...
snowluma_qq_start() {
    ...
    if [[ "$mode" == "docker" ]]; then
        if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$SNOWLUMA_DOCKER_NAME"; then
            error "Docker 容器未运行，请先启动 SnowLuma"
            return 1
        fi
        info "Docker 模式下通过 supervisor 拉起 QQ..."
        ...
        sleep 2
        if snowluma_qq_is_running; then
            info "容器内 QQ 已启动；扫码请用 noVNC: http://$(napcat_get_server_ip):${SNOWLUMA_NOVNC_PORT}/"
        else
            warn "未能确认 QQ 进程，请 docker logs / VNC 排查"
        fi
        return 0
    fi

    if [[ ! -x "/opt/QQ/qq" ]]; then
        error "未安装宿主 QQ，请先「安装 QQ」"
        return 1
    fi

    # 已在扫码屏上正常跑：只补 VNC/noVNC
    if snowluma_qq_is_running && snowluma_qq_on_mk_display && snowluma_xvfb_is_up; then
        warn "QQ 已在扫码屏 ${SNOWLUMA_DISPLAY} 运行"
        snowluma_vnc_start_stack || true
        snowluma_show_vnc_info
        return 0
    fi
    ...
    if snowluma_qq_is_running; then
        warn "检测到脱管/残缺 QQ（未正确挂在 ${SNOWLUMA_DISPLAY} 或虚拟屏已停）"
        info "将结束旧进程，再连同扫码桌面重新启动..."
        ...
        if snowluma_qq_is_running; then
            error "无法结束脱管 QQ，请先用「彻底杀死 QQ」，并检查是否有 systemd/外部守护在拉起"
    ...
    sleep 3
    if snowluma_qq_is_running; then
        info "QQ 已后台启动（DISPLAY=${SNOWLUMA_DISPLAY}）"
        snowluma_show_vnc_info
        info "请用手机扫 noVNC 里 QQ 窗口的二维码"
    else
        error "QQ 启动失败，见日志: ${SNOWLUMA_QQ_LOG}"
...
snowluma_qq_stop() {
    ...
    info "正在彻底停止宿主 QQ（含子进程，并短时压制自动拉起）..."
    snowluma_qq_disable_autostart_hints || true
    snowluma_qq_kill_all || true

    if snowluma_qq_is_running; then
        warn "仍检测到 /opt/QQ/qq，再次强杀..."
        snowluma_qq_kill_all || true
    fi
    ...
    if snowluma_qq_is_running; then
        error "QQ 仍在运行（可能被 systemd/NapCat 自启或其他守护拉起）"
        warn "排查: systemctl status ${AUTOSTART_NAPCAT_UNIT}"
        warn "排查: ps -ef | grep -E '/opt/QQ|qq' | grep -v grep"
        return 1
    fi

    info "宿主 QQ 已停止；远程查看/虚拟屏已关闭"
...
snowluma_qq_force_kill() {
    ...
    mode="$(snowluma_get_mode)"
    title "彻底杀死 QQ"
    echo "  将强制结束宿主 /opt/QQ 全部相关进程（含子进程/crashpad）"
    echo "  并在约 15 秒内反复压制自动拉起"
    echo "  默认不关闭 VNC/虚拟屏（需要可再点「仅停止远程查看」）"
    echo "  不会卸载 QQ 软件包"
    echo
    if [[ "$mode" == "docker" ]]; then
        echo "  当前为 Docker 模式：将在容器内强杀 QQ"
    fi
    read -r -p "确认彻底杀死? [y/N]: " confirm
    [[ "$confirm" =~ ^[Yy]$ ]] || { info "已取消"; return 0; }
    ...
    if snowluma_qq_is_running || pgrep -f '/opt/QQ/qq' >/dev/null 2>&1; then
        error "压制后仍有 /opt/QQ/qq，请检查外部守护："
        ps -ef | grep -E '/opt/QQ|mk-napcat' | grep -v grep || true
        warn "可尝试: sudo systemctl stop ${AUTOSTART_NAPCAT_UNIT} && sudo systemctl disable ${AUTOSTART_NAPCAT_UNIT}"
        return 1
    fi
    info "QQ 已彻底杀死"
    info "之后请用菜单「启动 QQ」重新拉起（才会带上 VNC DISPLAY）"
}
...
snowluma_qq_uninstall() {
    ...
    mode="$(snowluma_get_mode)"
    if [[ "$mode" == "docker" ]]; then
        warn "Docker 模式 QQ 在镜像内，卸载宿主包不会移除容器内 QQ"
        warn "若要彻底去掉 Docker QQ，请卸载 SnowLuma 容器"
    fi
    echo "  将卸载系统包 linuxqq（/opt/QQ）"
    echo
    local confirm=""
    read -r -p "确认卸载宿主 QQ? [y/N]: " confirm
    [[ "$confirm" =~ ^[Yy]$ ]] || { info "已取消"; return 0; }
    ...
    if [[ -d /opt/QQ ]]; then
        local wipe=""
        read -r -p "/opt/QQ 仍在，是否强制删除目录? [y/N]: " wipe
...
snowluma_start() {
    ...
    if [[ "$mode" == "docker" ]]; then
        ...
        if snowluma_is_running; then
            ...
            error "启动失败: docker logs ${SNOWLUMA_DOCKER_NAME}"
            return 1
        fi
        return 0
    fi
    ...
    if [[ ! -x "${SNOWLUMA_HOME}/launcher.sh" && ! -f "${SNOWLUMA_HOME}/index.mjs" ]]; then
        error "未找到 ${SNOWLUMA_HOME}/launcher.sh"
        return 1
    fi
    if snowluma_is_running; then
        warn "SnowLuma 已在运行"
...
snowluma_show_logs() {
    ...
    mode="$(snowluma_get_mode)"
    if [[ "$mode" == "docker" ]]; then
        title "SnowLuma Docker 日志 (最近 80 行)"
        docker logs --tail 80 "$SNOWLUMA_DOCKER_NAME" 2>&1 || warn "无法读取容器日志"
        echo
        echo "  实时: docker logs -f ${SNOWLUMA_DOCKER_NAME}"
        return 0
    fi
    if [[ ! -f "$SNOWLUMA_LOG" ]]; then
        warn "暂无运行日志: ${SNOWLUMA_LOG}"
        return 1
    fi
    title "SnowLuma 运行日志 (最近 80 行)"
    tail -n 80 "$SNOWLUMA_LOG"
    echo
    echo "  完整日志: ${SNOWLUMA_LOG}"
}
...
snowluma_show_login_info() {
    ...
    host_ip="$(napcat_get_server_ip)"
    mode="$(snowluma_get_mode)"
    title "SnowLuma WebUI 登录信息"
    echo "  模式:     ${mode:-未知}"
    echo "  版本:     $(snowluma_get_display_version || echo 未知)"
    echo "  WebUI:    http://${host_ip}:${SNOWLUMA_WEBUI_PORT}/"
    echo "  用户名:   admin"
    echo "  noVNC:    http://${host_ip}:${SNOWLUMA_NOVNC_PORT}/   (扫码登录 QQ)"
    echo "  VNC:      ${host_ip}:${SNOWLUMA_VNC_PORT}  密码 ${SNOWLUMA_VNC_PASS}"
    if [[ "$mode" == "docker" ]]; then
        ...
        echo "  密码见启动日志: ${SNOWLUMA_LOG}"
        echo "  安装目录: ${SNOWLUMA_HOME}"
        if snowluma_vnc_is_running; then
            echo -e "  扫码桌面: ${GREEN}运行中${NC}（常规安装已内置 Docker 同款 VNC/noVNC）"
        else
            echo -e "  扫码桌面: ${YELLOW}未运行${NC}（「启动 QQ」会自动拉起）"
        fi
    fi
    echo
}
...
menu_snowluma() {
    while true; do
        ...
        echo "  [2] 安装 SnowLuma"
        echo "  [3] 卸载 SnowLuma"
        echo "  [4] 安装 QQ（按系统/版本列兼容下载地址）"
        echo "  [5] 启动 QQ（常规模式会同时拉起 VNC/noVNC 扫码桌面）"
        echo "  [6] 停止 QQ"
        echo "  [7] 彻底杀死 QQ（含子进程，防拉起约 15 秒）"
        echo "  [8] 卸载 QQ"
        echo "  [9] 启动 SnowLuma"
        echo "  [10] 停止 SnowLuma"
        echo "  [11] 重启 SnowLuma"
        echo "  [12] 查看日志"
        echo "  [13] 查看 WebUI 登录信息"
        echo "  [14] 查看扫码地址 (VNC/noVNC)"
        echo "  [15] 仅启动远程查看 (VNC/noVNC)"
        echo "  [16] 仅停止远程查看 (不影响 QQ)"
        echo "  [17] 配置扫码端口/密码"
        echo
        snowluma_vnc_load_cfg
        if snowluma_is_running; then
            echo -e "  ${GREEN}* SnowLuma 运行中 (${mode:-?})${NC}"
        elif snowluma_is_installed; then
            echo -e "  ${YELLOW}* SnowLuma 已安装未运行 (${mode:-?})${NC}"
        else
            echo -e "  ${YELLOW}* SnowLuma 未安装${NC}"
        fi
        if snowluma_qq_is_running; then
            if snowluma_qq_on_mk_display && snowluma_xvfb_is_up; then
                echo -e "  ${GREEN}* QQ 运行中（扫码屏 ${SNOWLUMA_DISPLAY}）${NC}"
            else
                echo -e "  ${YELLOW}* QQ 脱管运行中（未挂扫码屏，点「启动 QQ」可自动修复）${NC}"
            fi
        elif snowluma_qq_is_installed; then
            echo -e "  ${YELLOW}* QQ 已安装未运行${NC}"
        fi
        if [[ "$mode" == "docker" ]] || snowluma_vnc_is_running; then
            echo -e "  ${GREEN}* 扫码桌面 VNC/noVNC 可用 :${SNOWLUMA_NOVNC_PORT}${NC}"
        elif snowluma_xvfb_is_up; then
            echo -e "  ${YELLOW}* 虚拟屏已开，远程查看未开（可点「仅启动远程查看」）${NC}"
        fi
        echo
        ...
        case "$choice" in
            ...
            15)
                if [[ "$(snowluma_get_mode)" == "docker" ]]; then
                    info "Docker 模式扫码桌面由容器提供，启动 SnowLuma 即可"
                else
                    snowluma_vnc_start_stack || true
                fi
                press_enter ;;
            16)
                if [[ "$(snowluma_get_mode)" == "docker" ]]; then
                    warn "Docker 模式请用「停止 SnowLuma」关闭容器；不能只停远程查看而保留容器内 QQ 屏"
                else
...
napcat_mod_check_version() {
    local ver=""
    if ! ver="$(napcat_mod_detect_version 2>/dev/null)"; then
        error "无法识别 NapCat 版本"
        error "魔改仅支持 ${NAPCAT_MOD_VER_MIN} - ${NAPCAT_MOD_VER_MAX}"
        return 1
    fi
    if ! napcat_mod_version_in_range "$ver"; then
        error "当前 NapCat 版本 v${ver} 不在支持范围"
        error "魔改仅支持 ${NAPCAT_MOD_VER_MIN} - ${NAPCAT_MOD_VER_MAX}"
        return 1
    fi
    return 0
...
napcat_mod_backup_mjs() {
    ...
    if [[ ! -f "$bak" ]]; then
        ...
        info "已备份 napcat.mjs -> napcat.mjs.bak"
    fi
}

napcat_mod_require_ready() {
    if ! napcat_mod_get_root; then
        error "NapCat 未安装或目录结构异常"
        return 1
    fi
    ...
    if ! napcat_mod_resolve_patches_dir >/dev/null; then
        error "内置魔改资源解压失败，请重新安装 mk: sudo bash mk --install"
        return 1
    fi
    return 0
}
...
PY
    local rc=$?
    if [[ $rc -eq 0 ]]; then
        info "魔改白名单限制完成：非官方插件不再被拦截"
        if napcat_is_running; then
            warn "NapCat 正在运行，建议执行「重启框架」使改动生效"
        fi
        return 0
    fi
    error "白名单魔改失败：当前 NapCat 版本可能与补丁不匹配"
    return 1
}
...
PY
    local rc=$?
    if [[ $rc -eq 0 ]]; then
        info "魔改插件上传完成：已恢复 /Plugin/Import 与 /Plugin/Reload API"
        if napcat_is_running; then
            warn "NapCat 正在运行，建议执行「重启框架」使改动生效"
        fi
        return 0
    fi
    error "插件上传魔改失败：当前 NapCat 版本可能与补丁不匹配"
    return 1
}
...
napcat_mod_patch_refresh_ui() {
    ...
    if [[ -z "$index_file" ]]; then
        error "未找到 WebUI 主路由 index 资源（需含插件页路由）"
        return 1
    fi
    info "定位 WebUI 主路由: $(basename "$index_file")"
...
PY
    ...
    if [[ $rc -ne 0 ]]; then
        if [[ $rc -eq 3 ]]; then
            error "刷新按钮魔改失败：未在 WebUI 主路由中找到插件页 lazy import"
        else
            error "刷新按钮魔改失败：WebUI 路由结构与补丁不匹配"
        fi
        return 1
    fi

    if new_ver="$(napcat_mod_bump_sw_cache 2>/dev/null)"; then
        info "已刷新 Service Worker 缓存版本: ${new_ver}"
    else
        warn "未能自动 bump 缓存版本，若页面仍显示旧 UI 请强制刷新浏览器"
    fi

    info "魔改刷新插件按钮完成：打开页面仅加载列表，刷新按钮执行全量重载"
    if napcat_is_running; then
        warn "NapCat 正在运行，建议执行「重启框架」；浏览器请 Ctrl+F5 强刷插件页"
    fi
    return 0
}
...
napcat_mod_print_version_status() {
    ...
    if ver="$(napcat_mod_detect_version 2>/dev/null)"; then
        if napcat_mod_version_in_range "$ver"; then
            echo -e "  NapCat 版本: v${ver} ${GREEN}(支持)${NC}"
        else
            echo -e "  NapCat 版本: v${ver} ${RED}(不支持)${NC}"
        fi
        return
    fi
    echo -e "  NapCat 版本: ${RED}未知版本 (不支持)${NC}"
}
...
menu_experimental() {
    while true; do
        title "实验功能"
        show_nav_hint
        echo "  NapCat 插件系统魔改（内置补丁，自动检测，已魔改则跳过）"
        echo "  支持版本: ${NAPCAT_MOD_VER_MIN} - ${NAPCAT_MOD_VER_MAX}"
        echo
        ...
        if napcat_mod_get_root 2>/dev/null; then
            ...
            echo "  当前状态:"
            echo -e "    白名单限制: $(napcat_mod_color_status "$s1")"
            echo -e "    插件上传:   $(napcat_mod_color_status "$s2")"
            echo -e "    刷新按钮:   $(napcat_mod_color_status "$s3")"
            if ! napcat_mod_version_in_range "${NAPCAT_MOD_VERSION:-0.0.0}" 2>/dev/null; then
                echo
                echo -e "  ${YELLOW}* 当前版本不在支持范围，魔改操作将被拒绝${NC}"
            fi
        else
            echo -e "  ${YELLOW}* NapCat 未安装，魔改功能不可用${NC}"
        fi
        echo
        echo "  [2] 魔改白名单限制"
        echo "  [3] 魔改插件上传"
...
kakake_ensure_node_tools() {
    ...
    if ! command -v npm >/dev/null 2>&1; then
        error "当前 Node 环境缺少 npm: ${node_path}"
        return 1
    fi
...
kakake_export_npm_registry() {
    ...
    if [[ -z "$cur" || "$cur" == "undefined" \
        ...
        info "npm registry → ${npm_config_registry}（已避开误配的 Node 二进制镜像）"
...
kakake_watch_web_build() {
    ...
    echo
    info "正在构建 Web UI（进度面板模式）"
    info "按 Ctrl+C 可退出进度显示，后台构建不会停止"
    echo

    trap 'echo; info "已退出进度显示，构建仍在后台继续"; trap - INT; return 0' INT
...
kakake_ensure_web_build() {
    ...
    if kakake_is_portable; then
        if kakake_web_built; then
            info "便携版 Web UI 已随包提供 ($(kakake_web_label))"
            return 0
        fi
        error "便携版缺少 packages/web/dist，请重新解压便携包"
        return 1
    fi

    if [[ "$force" != "1" ]] && ! kakake_web_needs_rebuild 0; then
        info "Web UI 已是最新 ($(kakake_web_label))"
        return 0
    fi

    if kakake_web_build_running; then
        info "检测到 Web 构建仍在进行，恢复进度显示"
        if kakake_watch_web_build && kakake_web_built; then
            kakake_web_write_stamp || true
            info "Web UI 就绪 ($(kakake_web_label))"
            return 0
        fi
        return 1
    fi
    ...
    if [[ "$force" == "1" ]]; then
        args+=(build --force)
        info "强制重建 Web UI → packages/web/dist"
    else
        info "需要构建 Web UI → packages/web/dist"
    fi
...
KAKAKEWEBEOF
    ...
    if ! kakake_web_built; then
        error "构建结束但未找到 packages/web/dist/index.html"
        kakake_show_web_build_errors
        return 1
    fi
...
kakake_status_line() {
    ...
    if [[ "$code" =~ ^(200|204|301|302|303|307|308|401|403|404)$ ]]; then
        if kakake_web_built; then
            echo -e "  ${GREEN}* 咔咔珂运行正常 (HTTP ${code} · ${port})${NC}"
        else
            echo -e "  ${YELLOW}* 服务在响应但 Web UI 未构建 (HTTP ${code})，后台页面不可用${NC}"
            echo -e "  ${YELLOW}  → 请先 [6] 停止，再 [10] 构建，然后 [5] 启动${NC}"
        fi
        return
    fi

    if [[ "$code" == "503" ]]; then
        echo -e "  ${YELLOW}* 服务在跑但返回 503（多半 Web UI 未构建）${NC}"
        echo -e "  ${YELLOW}  → 请 [6] 停止 → [10] 构建 → [5] 启动${NC}"
        return
    fi

    if kakake_has_process || kakake_port_listening "$port"; then
        echo -e "  ${RED}* 进程/端口异常占用，HTTP 无响应 (code=${code})${NC}"
        echo -e "  ${YELLOW}  → 请先 [6] 停止咔咔，再 [5] 重新启动${NC}"
        return
    fi

    if kakake_is_installed; then
        echo -e "  ${YELLOW}* 咔咔珂已安装 (未运行)${NC}"
    else
        echo -e "  ${YELLOW}* 咔咔珂未安装${NC}"
    fi
}
...
menu_node_select_mirror() {
    ...
    while true; do
        title "选择 Node.js 下载镜像"
        show_nav_hint
        echo "  国内服务器建议选择镜像源加速下载"
        echo
            ...
            *)
                if [[ "$choice" =~ ^[0-9]+$ ]]; then
                    ...
                    if (( idx >= 0 && idx < ${#node_mirror_labels[@]} )); then
                        ...
                        info "已选择: ${node_mirror_labels[$idx]}"
                        return 0
                    fi
                fi
                warn "无效选项，请重新输入"
...
kakake_node_install() {
    ...
    if kakake_node_private_installed; then
        ...
        warn "mk 管理的 Node 已安装: ${ver:-未知}"
        read -r -p "是否重新安装 Node.js? [y/N]: " confirm
        [[ "$confirm" =~ ^[Yy]$ ]] || return 0
    elif sys_bin="$(kakake_resolve_node_bin 2>/dev/null)"; then
        sys_ver="$("$sys_bin" -v 2>/dev/null || echo 未知)"
        info "检测到系统已有 Node.js ${sys_ver}（${sys_bin}）"
        info "咔咔启动会直接使用它；也可继续安装一份 mk 私有 Node（启动时优先）"
        read -r -p "是否仍安装 mk 私有 Node.js? [y/N]: " confirm
        [[ "$confirm" =~ ^[Yy]$ ]] || return 0
    fi
    ...
    title "安装 Node.js"
    echo "  版本:     ${NODE_INSTALL_VERSION_LABEL}"
    echo "  镜像:     ${node_mirror_labels[$NODE_INSTALL_MIRROR_IDX]}"
    echo "  安装路径: ${KAKAKE_NODE_DIR}"
    echo "  咔咔珂启动将优先使用此 Node"
    echo
    warn "若服务器上已有其他 Node 项目，切换版本可能导致那些项目无法正常运行"
    echo

    read -r -p "确认安装 Node.js v${install_ver}? [y/N]: " confirm
    [[ "$confirm" =~ ^[Yy]$ ]] || {
        info "已取消安装"
        return 0
    }
    ...
    info "正在下载 Node.js v${install_ver} ..."
    info "下载地址: ${url}"
...
kakake_node_uninstall() {
    ...
    if ! kakake_node_private_installed && [[ ! -f "$KAKAKE_NODE_VER_FILE" ]]; then
        warn "未检测到 mk 安装的 Node.js"
        if kakake_node_installed; then
            info "系统全局 Node 仍可用: $(kakake_get_node_version)（不会被本操作卸载）"
        fi
        return 0
    fi
    ...
    local confirm=""
    read -r -p "确认卸载 mk 管理的 Node.js (v${installed_ver})? [y/N]: " confirm
    [[ "$confirm" =~ ^[Yy]$ ]] || {
        info "已取消卸载"
        return 0
    }
    ...
    if kakake_http_alive "$(kakake_get_admin_port)" || kakake_has_process; then
        warn "咔咔珂正在运行或残留进程占用，请先停止"
        return 1
    fi

    rm -rf "$KAKAKE_NODE_DIR"
    rm -f "$KAKAKE_NODE_VER_FILE" "$KAKAKE_NODE_MIRROR_FILE"
    info "Node.js 已卸载 (mk 管理版本)"
    warn "系统自带的 node 命令(若有)不受影响"
}
...
kakake_do_install() {
    ...
    title "安装咔咔珂"
    echo "  安装目录均为: ${KAKAKE_HOME}"
    echo "  重新安装会保留 data/ 与 plugins/"
    echo
    ...
    if kakake_is_installed; then
        local reinstall=""
        warn "检测到咔咔珂已安装 ($(kakake_edition_label))"
        info "重新安装将保留 data/ 与 plugins/，其余文件会被替换"
        read -r -p "是否重新安装为所选版本? [y/N]: " reinstall
        [[ "$reinstall" =~ ^[Yy]$ ]] || return 0
        kakake_stop || true
    fi
    ...
    local tmp_zip="/tmp/kakake-mk-download.zip"
    local tmp_dir="/tmp/kakake-mk-extract"
    local keep_dir="/tmp/kakake-mk-preserve"
    local preserved=0
    ...
    info "正在下载咔咔珂 (${zip_label})..."
    info "地址: ${download_url}"
    if [[ "$preserved" -eq 1 ]]; then
        info "将保留现有 data/ 与 plugins/"
    fi
    ...
    if ! kakake_is_installed; then
        error "安装失败，请查看: ${KAKAKE_INSTALL_LOG}"
    ...
    if [[ "$want_edition" == "portable" ]] && ! kakake_is_portable; then
        warn "已下载便携包，但目录未识别为便携版，请检查 zip 内容是否含 runtime/bin/node"
    elif [[ "$want_edition" == "source" ]] && ! kakake_is_source; then
        warn "已下载源码包，但目录未识别为源码版，请检查是否含 scripts/bootstrap.mjs"
    fi

    mkdir -p "$KAKAKE_DATA"
    kakake_open_admin_port
    info "咔咔珂安装成功: ${KAKAKE_HOME} ($(kakake_edition_label))"
    if [[ "$preserved" -eq 1 ]]; then
        info "已保留原 data/ 与 plugins/"
    fi
    info "已开放 ${KAKAKE_ADMIN_PORT} 端口，未自动启动"
    if kakake_is_portable; then
        info "当前为便携版（内置 Node），可直接「启动咔咔」，无需再装系统 Node"
    else
        info "请先「安装 Node.js」（若本机尚无），再「启动咔咔」"
    fi
}
...
kakake_start() {
    ...
    if [[ -z "$node_bin" ]]; then
        if kakake_is_portable; then
            error "便携版缺少 runtime/bin/node，请检查目录是否完整"
        else
    ...
    if kakake_http_alive "$port"; then
        if kakake_web_built; then
            warn "咔咔珂已在正常运行 ($(kakake_edition_label))"
            kakake_show_login_info
            return 0
        fi
        warn "服务在响应但 Web UI 未构建，将重启并先构建前端"
        kakake_stop || true
        sleep 1
    elif kakake_has_process || kakake_port_listening "$port"; then
        warn "检测到残留进程或端口占用但 HTTP 无响应，先清理…"
        kakake_stop || true
        # 仍占用则强杀监听进程
    ...
    info "正在后台启动咔咔珂（$(kakake_edition_label) · ${entry}）…"
    (
    ...
    if ! kakake_http_alive "$port"; then
        (( panel_active == 1 )) && kakake_start_panel_cursor_up
        error "等待超时（HTTP :${port} 无响应），请查看: ${KAKAKE_LOG}"
    ...
    kakake_open_admin_port
    info "咔咔珂启动成功 (后台运行 · $(kakake_edition_label))"
    kakake_show_login_info
    info "退出 mk 菜单不会停止咔咔珂，请使用「停止咔咔」关闭"
}
...
kakake_stop() {
    ...
    if command -v systemctl >/dev/null 2>&1; then
        if systemctl list-unit-files "${AUTOSTART_KAKAKE_UNIT}" >/dev/null 2>&1 \
            ...
            info "已停止 systemd 服务 ${AUTOSTART_KAKAKE_UNIT}（避免自动拉起）"
        fi
    fi
    ...
    if kakake_has_process || kakake_port_listening "$port" || kakake_http_alive "$port"; then
        ...
        if command -v systemctl >/dev/null 2>&1 && systemctl is-enabled "$AUTOSTART_KAKAKE_UNIT" >/dev/null 2>&1; then
            warn "检测到 ${AUTOSTART_KAKAKE_UNIT} 仍启用；若日志在刷 bootstrap 找不到，请到「开机自启」关闭后重装自启"
        fi
        warn "可手动: systemctl stop ${AUTOSTART_KAKAKE_UNIT}; fuser -k ${port}/tcp"
        return 1
    fi
...
kakake_show_login_info() {
    ...
    if kakake_web_built; then
        echo "  Web 构建: 已就绪 ($(kakake_web_label))"
    else
        echo "  Web 构建: 未完成（启动时会自动 build:web）"
    fi
    echo
    warn "若无法访问，请在安全组放行 ${port} 端口"
}
...
kakake_do_uninstall() {
    title "卸载咔咔珂"
    echo "  将删除 ${KAKAKE_HOME} 及运行状态"
    echo

    local confirm="" admin_port=""
    read -r -p "确认彻底卸载咔咔珂? [y/N]: " confirm
    [[ "$confirm" =~ ^[Yy]$ ]] || {
        info "已取消卸载"
        return 0
    }
...
LABELS = {
    "onebot:reverse": "OneBot 反向 (咔咔=Server)",
    "onebot:forward": "OneBot 正向 (咔咔=Client)",
...
kakake_conn_check_conflicts() {
    ...
    if [[ "$has_rev" -eq 1 && "$has_fwd" -eq 1 ]]; then
        warn "同时启用了「${rev_name}」(咔咔 Server) 和「${fwd_name}」(咔咔 Client)，通常只需一种"
        echo "  Server=正向(监听)  Client=反向(连出)"
        echo "  A 反向: 咔咔 Server :${rev_port} ← NapCat WebSocket Client"
        echo "  B 正向: 咔咔 Client → NapCat WebSocket Server :3001"
    fi

    if [[ "$has_fwd" -eq 1 && "$fwd_port" == "6700" ]]; then
        error "「${fwd_name}」正向 WS 指向 6700 有误"
        echo "  6700 是咔咔反向监听口；正向连 6700 会连到咔咔自己，不是 NapCat"
        echo "  日志里 jb「已连接」是假象，真正连 NapCat 的是「${rev_name:-反向连接}」"
    elif [[ "$has_fwd" -eq 1 && "$has_rev" -eq 1 && "$fwd_port" == "$rev_port" ]]; then
        error "正向与反向使用了同一端口 ${rev_port}，正向会连到咔咔自身"
    fi
}

kakake_show_pairing_hint() {
    echo -e "${CYAN}  术语: WebSocket Server=正向(监听)  Client=反向(连出)${NC}"
    echo "  A. NapCat Client(反向) → ws://127.0.0.1:6700  [咔咔=Server，反向]"
    echo "  B. NapCat Server(正向) ← ws://127.0.0.1:3001  [咔咔=Client，正向]"
    echo
}
...
kakake_conn_diagnose() {
    ...
    echo -e "${BOLD}【连接诊断】${NC} ${name:-连接}"
    echo

    if ! kakake_conn_is_enabled "$enable"; then
        warn "此连接未启用 (enable=false)"
        return 0
    fi
    if ! kakake_is_running; then
        error "咔咔珂未运行，请先: 咔咔操作 → 启动咔咔"
        return 0
    fi

    if [[ "$mode" == "reverse" ]]; then
        if kakake_reverse_ws_has_client "$port"; then
            info "正常: NapCat 已连入咔咔 ${host}:${port}"
        elif kakake_port_listening "$port"; then
            warn "咔咔正在 ${host}:${port} 监听，但 NapCat 尚未连入"
            echo "  NapCat 侧: 网络配置 → WebSocket Client (反向 WS)"
            echo "  连接地址: ws://127.0.0.1:${port}"
            echo "  Token 需与咔咔连接配置中的 Access Token 一致"
        else
            error "咔咔未在端口 ${port} 监听"
            echo "  请重启咔咔，或用「端口验证」检查 ${port} 是否被占用"
        fi
        return 0
    fi
    ...
    if [[ "$mode" == "forward" ]]; then
        ...
        if [[ "$port" == "6700" ]] && kakake_port_listening "6700"; then
            error "「${name}」正向 WS 连 ws://${host}:6700 实际连的是咔咔自己的反向监听"
            echo "  这不是 NapCat！日志「已连接」是连到本机 6700，数据未进 NapCat"
            echo "  NapCat 侧应配 WebSocket Client (反向) → ws://127.0.0.1:6700"
            echo "  由「NapCat 默认接入」等反向连接负责，请禁用或删除本正向连接 jb"
            kakake_show_pairing_hint
            return 0
        fi
        if kakake_forward_ws_has_session "$host" "$port"; then
            info "正常: 咔咔已连上 NapCat ws://${host}:${port}"
            return 0
        fi

        if kakake_port_listening "$port"; then
            if [[ "$port" == "6700" ]]; then
                error "6700 上有咔咔反向监听，正向 WS 不应连此端口"
            else
                warn "ws://${host}:${port} 有程序在监听，但 WebSocket 未握手成功"
            fi
            echo "  检查 Token 是否与 NapCat WebSocket Server (正向) 一致"
            echo "  查看日志: tail -f ${KAKAKE_LOG}"
            return 0
        fi

        error "连接被拒绝 (ECONNREFUSED): ws://${host}:${port} 没有任何服务在监听"
        echo
        echo "  正向 = 咔咔 Client 连 NapCat WebSocket Server (正向，NapCat 监听)"
        echo "  反向 = 咔咔 Server 监听，NapCat WebSocket Client (反向) 连入"
        echo
        if [[ "$port" == "6700" ]]; then
            warn "6700 是咔咔「反向 WS」的默认监听端口，不是 NapCat 的端口！"
            echo "  你当前配成了正向 WS 去连 6700，所以一定会 ECONNREFUSED"
            echo
            echo "  方案 A (推荐，常用): 改用反向 WS"
            echo "    1. 删除连接「${name}」，重新添加 → OneBot 反向 WS，端口 6700"
            echo "    2. NapCat → WebSocket Client (反向) → ws://127.0.0.1:6700"
            echo
            echo "  方案 B: 继续用正向 (咔咔 Client → NapCat Server)"
            echo "    1. NapCat → WebSocket Server (正向)，端口 3001"
            echo "    2. 咔咔 → OneBot 正向 (Client)，ws://127.0.0.1:3001"
        else
            echo "  请确认:"
            echo "    1. NapCat 已启动 (NapCat操作 → 启动框架)"
            echo "    2. NapCat 已添加 WebSocket Server (正向)，端口 ${port} (常用 3001)"
            echo "    3. 咔咔 OneBot 正向 (Client) 地址与 NapCat Server 端口一致"
        fi
        return 0
    fi

    info "请查看咔咔控制台日志获取更多信息"
}
...
menu_kakake_conn_add() {
    ...
    while true; do
        title "新增连接 - 选择类型"
        show_nav_hint
        echo "  [2] OneBot 反向 (咔咔=Server) ← NapCat WebSocket Client"
        echo "  [3] OneBot 正向 (咔咔=Client) → NapCat WebSocket Server"
        echo "  [4] QQ 官方机器人"
        echo
    ...
    if [[ "$kind" == "qq_official" ]]; then
        ...
        read -r -p "监听地址 [0.0.0.0]: " host
        host="${host:-0.0.0.0}"
        read -r -p "监听端口 [6700]: " port
        port="${port:-6700}"
        read -r -p "Access Token (留空跳过): " token
        read -r -p "是否启用? [Y/n]: " enable
        [[ "$enable" =~ ^[Nn]$ ]] && enable="n" || enable="y"
        ...
        read -r -p "NapCat WS 地址 [127.0.0.1]: " host
        host="${host:-127.0.0.1}"
        read -r -p "NapCat WS 端口 [3001]: " port
        port="${port:-3001}"
        if [[ "$port" == "6700" ]]; then
            echo
            error "6700 是咔咔「反向 WS」监听端口，不是 NapCat 服务端端口"
            echo "  若 NapCat 配 WebSocket Server (正向)，咔咔应选 OneBot 正向 (Client)，端口 3001"
            echo "  若要用 6700，咔咔选 OneBot 反向 (Server)，NapCat 配 WebSocket Client (反向)"
            read -r -p "是否改用 3001? [Y/n]: " fix_port
    ...
    info "连接配置已保存: ${KAKAKE_CONNECTIONS_FILE}"
    info "配置已写入，重启咔咔后可确保连接完全生效"
}
    ...
    while true; do
        ...
        kakake_cfg_tool get "$idx" 2>/dev/null | while IFS=$'\t' read -r key val; do
            case "$key" in
                ...
                mode)
                    if [[ "$val" == "forward" ]]; then
                        echo "  模式:     正向 (咔咔 Client → NapCat Server)"
                    elif [[ "$val" == "reverse" ]]; then
                        echo "  模式:     反向 (咔咔 Server ← NapCat Client)"
                    else
                        echo "  模式:     ${val}"
                    fi
                    ;;
...
menu_kakake_configure() {
    ...
    while true; do
        ...
        echo
        info "配置文件: ${KAKAKE_CONNECTIONS_FILE}"
        echo
...
menu_kakake() {
    while true; do
        ...
        kakake_status_line
        if kakake_is_installed; then
            echo -e "  ${GREEN}* 安装形态: $(kakake_edition_label)${NC}"
        fi
        if [[ -n "$node_bin" ]]; then
            ...
            if kakake_is_portable; then
                echo -e "  ${YELLOW}  → 便携版应自带 runtime/bin/node，请检查目录完整性${NC}"
            fi
        fi
        if kakake_is_installed; then
            port="$(kakake_get_admin_port)"
            if kakake_web_built; then
                echo -e "  ${GREEN}* Web UI 已构建 · 端口 ${port}${NC}"
            else
                echo -e "  ${YELLOW}* Web UI 未构建 · 端口 ${port}${NC}"
                if kakake_is_portable; then
                    echo -e "  ${YELLOW}  → 便携版缺 dist，请重新解压完整便携包${NC}"
                else
                    echo -e "  ${YELLOW}  → 选 [10] 构建，或 [5] 启动时会先构建${NC}"
                fi
            fi
        fi
        echo
        ...
        case "$choice" in
            ...
            10)
                if kakake_is_portable; then
                    warn "便携版 Web UI 已随包提供，无需/无法在此重建"
                    if kakake_web_built; then
                        info "当前 dist 正常: ${KAKAKE_WEB_DIST_INDEX}"
                    else
                        error "缺少 packages/web/dist，请重新解压便携包"
                    fi
                else
                    node_bin="$(kakake_resolve_node_bin 2>/dev/null || true)"
                    if [[ -z "$node_bin" ]]; then
...
font_ensure_fc() {
    ...
    warn "未检测到 fontconfig（fc-list），尝试安装..."
    if command -v apt-get >/dev/null 2>&1; then
        apt-get update -qq >/dev/null 2>&1 || true
...
font_pkg_status_line() {
    ...
    if command -v dpkg >/dev/null 2>&1; then
        if dpkg -s "$name" 2>/dev/null | grep -qi '^Status:.*installed'; then
            echo -e "  ${GREEN}* 软件包 ${name}: 已安装${NC}"
            return 0
        fi
        echo -e "  ${YELLOW}* 软件包 ${name}: 未安装${NC}"
        return 1
    fi
    if command -v rpm >/dev/null 2>&1; then
...
font_show_status() {
    ...
    if command -v fc-list >/dev/null 2>&1; then
        if families="$(font_scan_fc_zh 2>/dev/null)"; then
            ...
            echo -e "  ${GREEN}* 已检测到中文字体族 ${count} 个（fontconfig）${NC}"
            printf '%s\n' "$families" | head -n 12 | while IFS= read -r line; do
                [[ -n "$line" ]] && echo "    - $line"
            done
            if (( count > 12 )); then
                echo "    ... 另有 $((count - 12)) 个未列出"
            fi
        else
            echo -e "  ${YELLOW}* fontconfig 未列出中文字体族${NC}"
        fi
    else
        echo -e "  ${YELLOW}* 未安装 fontconfig（无法用 fc-list 精确检测）${NC}"
    fi
...
font_refresh_cache() {
    ...
    if command -v fc-cache >/dev/null 2>&1; then
        ...
        warn "无 fc-cache，跳过刷新；新字体可能需重启相关进程后生效"
    fi
}
...
menu_font_select_mirror() {
    ...
    while true; do
        title "选择字体下载镜像源"
        show_nav_hint
        echo "  无外网请返回后选「从本地文件安装」"
        echo "  国内服务器访问 GitHub 困难时，请选代理镜像"
        echo
        for ((i=0; i<${#font_mirror_labels[@]}; i++)); do
            idx=$((i + 2))
            echo "  [${idx}] ${font_mirror_labels[$i]}"
        done
        echo "  [$(( ${#font_mirror_labels[@]} + 2 ))] 自定义完整下载 URL"
        echo
            ...
            *)
                if [[ "$choice" =~ ^[0-9]+$ ]]; then
                    ...
                    if (( choice == custom_idx )); then
                        local url=""
                        read -r -p "请粘贴字体文件完整 URL: " url </dev/tty 2>/dev/null || read -r -p "请粘贴字体文件完整 URL: " url
                        url="${url#"${url%%[![:space:]]*}"}"
                        url="${url%"${url##*[![:space:]]}"}"
                        if [[ -z "$url" ]]; then
                            warn "URL 为空"
                    ...
                    if (( idx >= 0 && idx < ${#font_mirror_labels[@]} )); then
                        ...
                        FONT_CUSTOM_URL=""
                        info "已选择: ${font_mirror_labels[$idx]}"
                        return 0
                    fi
                fi
                warn "无效选项，请重新输入"
...
font_install_via_download() {
    ...
    if head -c 200 "$tmp" 2>/dev/null | grep -qiE '<html|<!DOCTYPE'; then
        error "下载内容像网页而非字体文件，请换镜像或自定义 URL"
        rm -f "$tmp"
        return 1
    fi
    mkdir -p "$FONT_INSTALL_DIR"
...
font_install_via_packages() {
    require_root || return 1
    info "通过系统软件源安装中文字体（走本机 apt/yum 镜像，适合已配国内源的机器）"
    echo
    local ok=0
    if command -v apt-get >/dev/null 2>&1; then
        ...
        error "未识别到 apt / dnf / yum / pacman，请改用下载安装或本地文件"
        return 1
    fi
    ...
    error "软件源安装可能失败（仓库无包或无外网）。可改用镜像下载，或把字体文件拷到服务器后「从本地文件安装」"
    return 1
}
...
font_install_from_local() {
    require_root || return 1
    local path=""
    echo "  支持 .ttf / .otf / .ttc；可先 scp 到服务器再填写路径"
    echo "  例: /root/wqy-microhei.ttc"
    echo
    read -r -p "本地字体文件路径: " path </dev/tty 2>/dev/null || read -r -p "本地字体文件路径: " path
...
menu_fonts() {
    while true; do
        ...
        show_nav_hint
        if font_has_cjk; then
            echo -e "  ${GREEN}* 状态: 已检测到中文字体${NC}"
        else
            echo -e "  ${YELLOW}* 状态: 未检测到中文字体（截图/中文渲染可能异常）${NC}"
        fi
        echo
        echo "  [2] 扫描检测中文字体"
        echo "  [3] 软件源一键安装（apt/yum，走系统镜像）"
        echo "  [4] 下载安装 文泉驿微米黑（体积较小）"
        echo "  [5] 下载安装 Noto Sans CJK SC（覆盖更好）"
        echo "  [6] 从本地文件安装（无外网 / 自备字体）"
        echo "  [7] 刷新字体缓存（fc-cache）"
        echo
        echo "  说明: [4]/[5] 可选国内代理或自定义 URL；完全断网请用 [6]"
        echo
...
port_guess_service_hint() {
    ...
    case "$port" in
        ...
        6700)  echo "常见用途: 咔咔 OneBot 反向 WS" ;;
...
port_show_verify_summary() {
    ...
    echo -e "  $(printf '%-6s' "$port")  占用:${listen_text}  连接:${conn_text}  防火墙:${fw_text}  mk:${cfg_text}"
}
...
port_scan_mk_ports() {
    ...
    title "mk 相关端口扫描 (共 ${#ports[@]} 个)"
    echo "  端口    占用        连接      防火墙      mk配置"
    echo "  --------------------------------------------------------------"
    for p in "${ports[@]}"; do
    ...
    info "端口列表: ${ports[*]}"
    echo
    read -r -p "输入端口号查看详情 (直接回车返回): " choice
    choice="${choice//$'\r'/}"
...
port_list_all_listeners() {
    ...
    title "本机全部监听端口"
    if ! command -v ss >/dev/null 2>&1; then
        error "需要 ss 命令，无法列出端口"
        return 1
    fi

    echo -e "  ${BOLD}端口    协议        状态      监听地址              进程${NC}"
    echo "  ----------------------------------------------------------------------"
    ...
    echo
    info "共上表为当前监听/未连接 UDP 端口；输入具体端口号可查看详情"
}
...
port_show_config_refs() {
    ...
    if declare -f napcat_cfg_tool >/dev/null 2>&1; then
        while IFS=$'\t' read -r _ _ name addr _ cat; do
            ...
            if [[ "$(port_parse_addr_port "$addr")" == "$port" ]]; then
                echo "  NapCat 网络配置: ${name} (${cat}) -> ${addr}"
                found=1
            fi
        done < <(napcat_cfg_tool list 2>/dev/null || true)
    fi
    ...
    if [[ "$port" == "$KAKAKE_ADMIN_PORT" ]]; then
        echo "  咔咔珂控制台默认端口 ${KAKAKE_ADMIN_PORT}"
        found=1
    fi

    if [[ "$found" -eq 0 ]]; then
        echo "  mk 管理的配置中未发现此端口"
    fi
}
...
port_firewall_check() {
    ...
    if command -v firewall-cmd >/dev/null 2>&1; then
        ...
        if echo " $fw_ports " | grep -qE " ${port}/tcp "; then
            echo -e "  firewalld: ${GREEN}已放行 TCP ${port}${NC}"
            found=1
        elif echo " $fw_ports " | grep -qE " ${port}/udp "; then
            echo -e "  firewalld: ${GREEN}已放行 UDP ${port}${NC}"
            found=1
        else
            echo -e "  firewalld: ${YELLOW}未放行 ${port}${NC}"
        fi
    fi

    if command -v iptables >/dev/null 2>&1; then
        if iptables -C INPUT -p tcp --dport "$port" -j ACCEPT >/dev/null 2>&1; then
            echo -e "  iptables INPUT: ${GREEN}已放行 TCP ${port}${NC}"
            found=1
        elif iptables -L INPUT -n 2>/dev/null | grep -qE "dpt:${port}([[:space:]]|$)"; then
            echo -e "  iptables INPUT: ${GREEN}存在 ${port} 相关规则${NC}"
            found=1
        elif [[ "$ufw_active" -eq 0 ]]; then
            echo -e "  iptables: ${YELLOW}未找到 ${port} 放行规则${NC}"
        fi
    fi
    ...
    if command -v nft >/dev/null 2>&1; then
        ...
        if nft list ruleset 2>/dev/null | grep -qE "dport[[:space:]]*(${port}|0x${port_hex}|${port_hex})"; then
            echo -e "  nftables: ${GREEN}存在 ${port} 相关规则${NC}"
            found=1
        fi
    fi
    ...
    if [[ "$found" -eq 0 ]] \
        ...
        echo "  未检测到 ufw / firewalld / iptables / nftables"
    fi
}
...
port_show_listeners() {
    ...
    if command -v ss >/dev/null 2>&1; then
        while IFS= read -r line; do
            ...
            proc="${proc:-未知}"
            echo "  协议:     $(port_translate_proto "$proto")"
            echo "  状态:     $(port_translate_state "$state")"
            echo "  监听地址: $(port_translate_addr "$host") (${local_addr})"
            echo "  进程:     $(port_translate_process "$proc")"
            [[ -n "$pid" ]] && echo "  进程 ID:  ${pid}"
            [[ "$proc" == "未知" && $EUID -ne 0 ]] && echo "  提示:     使用 sudo mk 可查看其他用户进程"
            echo "  ---"
        done < <({
    ...
    if [[ "$has_listener" -eq 0 ]] && port_proc_has_listener "$port"; then
        has_listener=1
        echo -e "  ${GREEN}内核显示端口 ${port} 已被占用${NC}"
        echo "  说明: 检测到 /proc/net 中有此端口，但未能解析进程名"
        [[ $EUID -ne 0 ]] && echo "  建议: 使用 sudo mk 或 sudo ss -tulnp | grep :${port}"
    fi

    if [[ "$has_listener" -eq 0 ]]; then
        echo -e "  ${YELLOW}当前没有进程监听此端口${NC}"
        if port_has_established "$port"; then
            echo "  说明: 无监听，但存在指向此端口的已建立连接"
        else
            echo "  说明: 端口可能空闲，或服务尚未启动"
        fi
    fi
}
...
port_show_connections() {
    ...
    for state in "${!state_map[@]}"; do
        echo "    $(port_translate_state "$state"): ${state_map[$state]} 条"
    done
}
...
port_show_verify_report() {
    ...
    if [[ "$listening" -eq 1 ]]; then
        echo -e "  占用状态: ${GREEN}使用中 (有程序正在监听)${NC}"
    elif [[ "$established" -eq 1 ]]; then
        echo -e "  占用状态: ${YELLOW}有连接活动 (无监听，可能是出站连接)${NC}"
    else
        echo -e "  占用状态: ${YELLOW}未发现监听或连接${NC}"
    fi
    ...
    echo -e "${BOLD}【本机防火墙】${NC}"
    port_firewall_check "$port"
    echo

    warn "云服务商「安全组」无法在此检测，外网访问失败时请去控制台放行"
    info "手动验证: ss -tulnp '( sport = :${port} )'  或  sudo lsof -i :${port}"
}
...
port_kill_process_flow() {
    ...
    title "杀掉进程"
    [[ $EUID -ne 0 ]] && warn "当前非 root，可能无法结束其他用户的进程（建议 sudo mk）"
    echo
    read -r -p "请输入要清理的端口号: " port
    port="${port//$'\r'/}"
    ...
    while true; do
        ...
        title "端口 ${port} 占用进程"
        if [[ ${#pids[@]} -eq 0 ]]; then
            info "端口 ${port} 当前没有可识别的占用进程"
            [[ $EUID -ne 0 ]] && echo "  提示: 使用 sudo mk 可查看更多进程信息"
            return 0
        fi

        echo -e "  ${BOLD}序号  PID       协议        状态          地址                    进程${NC}"
        echo "  --------------------------------------------------------------------------------"
        for i in "${!pids[@]}"; do
        ...
        echo
        echo -e "  ${YELLOW}[2] 全部杀掉${NC}"
        echo "  输入上方序列号杀掉对应进程；输入 2 全部杀掉；输入 q 返回"
        echo
        ...
        if [[ "$choice" == "2" ]]; then
            echo
            warn "即将结束端口 ${port} 上的全部 ${#pids[@]} 个进程"
            read -r -p "确认全部杀掉? [y/N]: " choice
            choice="$(normalize_choice "${choice:-}")"
        ...
        warn "无效输入，请输入列表中的序列号，或输入 2 全部杀掉"
    done
}
...
menu_port_verify() {
    while true; do
        title "端口验证"
        show_nav_hint
        echo "  [2] 扫描 mk 相关端口 (NapCat/咔咔/宝塔等，自动)"
        echo "  [3] 列出本机全部监听端口"
        echo "  [4] 杀掉进程 (按端口列出并可结束)"
        echo
        echo "  也可直接输入端口号 (任意 1-65535，支持多个):"
        echo "  示例: 8700  或  3001,8787,6099  或  8080 9000"
        echo "  输入 q 返回上一步，0 返回首页"
        echo

        local port_input=""
        read -r -p "选项或端口号: " port_input
...
mkbot_show_node_status() {
    ...
    if bin="$(mkbot_resolve_node_bin 2>/dev/null)"; then
        ...
        if mkbot_node_version_ok "$bin"; then
            echo -e "  ${GREEN}* Node.js ${ver} (满足构建要求 20+)${NC}"
        else
            echo -e "  ${YELLOW}* Node.js ${ver} (版本过低，需要 20+)${NC}"
        fi
    else
        echo -e "  ${YELLOW}* Node.js 未安装 (构建需要 Node 20+)${NC}"
    fi
}
...
mkbot_watch_build() {
    ...
    if [[ -d "$out" && -f "${out}/index.mjs" ]]; then
        info "构建成功"
        echo "  成品目录: ${out}"
        echo "  入口文件: ${out}/index.mjs"
        echo "  插件清单: ${out}/plugin.json"
        echo
        info "可将 ${MKBOT_OUT_SUBDIR} 目录部署到 NapCat/咔咔 插件目录"
    else
        warn "构建进程已结束，但未找到预期成品目录"
        info "请检查: ${MKBOT_BUILD_LOG}"
        return 1
    fi
    return 0
}
...
mkbot_start_build() {
    ...
    if [[ -z "$node_bin" ]]; then
        error "未找到 Node.js，请先「安装 Node.js」(需要 20+)"
        return 1
    fi
...
mkbot_prompt_and_build() {
    ...
    echo
    read -r -p "MK 插件源码路径 (含 package.json 的目录): " raw
    ...
    if ! src_root="$(mkbot_find_project_root "$raw")"; then
        error "未识别为 MKbot 源码目录: ${raw}"
        echo "  请指向解压后的 MK 目录 (含 package.json 与 vite.config.ts)"
        echo "  若解压为 MKbot.zip/MK/ 则路径可填上级或 MK 子目录"
        return 1
    fi

    if [[ "$src_root" != "$raw" ]]; then
        info "已识别源码根目录: ${src_root}"
    fi
...
menu_mkbot_build() {
    while true; do
        title "构建 MK 源码"
        show_nav_hint
        echo "  [2] 安装 Node.js (构建需要 20+，与咔咔共用)"
        echo "  [3] 卸载 Node.js"
        echo "  [4] 输入源码路径并开始构建"
        if mkbot_build_running; then
            echo "  [5] 查看构建进度"
...
autostart_napcat_enable() {
    ...
    napcat_detect_paths || return 1
    if ! napcat_ensure_xvfb; then
        error "缺少 xvfb-run，无法配置 NapCat 开机自启"
        return 1
    fi
...
EOF
    ...
    if napcat_is_running; then
        systemctl reset-failed "$AUTOSTART_NAPCAT_UNIT" >/dev/null 2>&1 || true
        info "NapCat 开机自启已开启，当前运行中"
        napcat_open_port_6099
        return 0
    fi
    if autostart_unit_failed "$AUTOSTART_NAPCAT_UNIT"; then
        error "NapCat 开机自启已写入，但启动失败，请查看: ${NAPCAT_LOG}"
        journalctl -u "$AUTOSTART_NAPCAT_UNIT" -n 10 --no-pager 2>/dev/null || true
        return 1
    fi
    warn "NapCat 开机自启已开启，服务正在拉起中..."
}
...
EOF
    ...
    if kakake_http_alive "$(kakake_get_admin_port)"; then
        systemctl reset-failed "$AUTOSTART_KAKAKE_UNIT" >/dev/null 2>&1 || true
        info "咔咔珂开机自启已开启，当前运行中 ($(kakake_edition_label))"
        kakake_open_admin_port
        kakake_show_login_info
        return 0
    fi
    if kakake_has_process; then
        warn "咔咔珂开机自启已开启，进程在拉起中（可能正在构建），请稍候查看日志: ${KAKAKE_LOG}"
        return 0
    fi
    if autostart_unit_failed "$AUTOSTART_KAKAKE_UNIT"; then
        error "咔咔珂开机自启已写入，但启动失败，请查看: ${KAKAKE_LOG}"
        journalctl -u "$AUTOSTART_KAKAKE_UNIT" -n 10 --no-pager 2>/dev/null || true
        return 1
    fi
    warn "咔咔珂开机自启已开启，服务正在拉起中..."
}
...
menu_autostart() {
    while true; do
        ...
        echo
        echo "  选择序号开启/关闭自启（已开启时再次选择则关闭）"
        echo "  颜色: 白=未开启  绿=运行中  红=失败  黄=咔咔无接通连接"
        echo
        ...
        case "$choice" in
            0) return 0 ;;
            1) autostart_toggle_napcat || true; press_enter ;;
...
main() {
    case "${1:-}" in
        ...
        --check|-c)
            ...
            ;;
        --version|-v)
            echo "mk 版本: ${MK_VERSION}"
            ;;
        --help|-h)
            echo "用法:"
            echo "  mk / MK / Mk / mK  打开交互菜单（字母大小写均可）"
            echo "  sudo bash mk      首次安装/更新"
            echo "  mk --install      强制重装/更新"
            echo "  mk --uninstall    卸载 mk 工具"
            echo "  mk --check        语法检查"
            echo "  mk --version      查看版本"
            echo "  mk --patch-all            应用全部实验魔改"
            ;;
        --patch-all)
            if [[ "$SELF_PATH" != "${INSTALL_DIR}/${SCRIPT_FILE}" ]]; then
                auto_install_if_needed
